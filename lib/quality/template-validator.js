...
function detectDocumentType(filePath) {
  ...
  if (filePath.includes('/plan/') || filePath.includes('.plan.md')) return 'plan';
  if (filePath.includes('/design/') || filePath.includes('.design.md')) return 'design';
  if (filePath.includes('/do/') || filePath.includes('.do.md')) return 'do';
  if (filePath.includes('/qa/') || filePath.includes('.qa.md')) return 'qa';
  if (filePath.includes('/report/') || filePath.includes('.report.md')) return 'report';
...
function validateDocument(filePath, content) {
  ...
  const missing = required.filter(section => {
    ...
    return !actual.some(a => {
      ...
      // Exact match or starts with the required section name (word boundary)
      return aLower === sectionLower || aLower.startsWith(sectionLower + ' ') || aLower.startsWith(sectionLower + ':');
    });
  });
