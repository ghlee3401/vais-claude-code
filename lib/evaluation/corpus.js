...
function compileExpectedPhaseGraph(profile, signals = {}) {
  ...
  if (profile === 'feature') {
    const designRequired = DESIGN_SIGNALS.some(signal => signals[signal] === true);
  ...
  if (profile === 'initiative') {
    return signals.ceoAnalysisAvailable === true
      ? {
        required: ['plan', 'design', 'do', 'qa', 'report'],
        conditional: ['ideation'],
        notRequired: [],
      }
      : {
        required: ['ideation', 'plan', 'design', 'do', 'qa', 'report'],
        conditional: [],
        notRequired: [],
      };
  }
...
function validateClassificationCorpus(corpus) {
  ...
  push(errors, corpus?.splitPolicy?.heldOutIdsHash === heldOutIdsHash, 'splitPolicy.heldOutIdsHash must match held-out ids');
  ...
  for (const profile of PROFILES) {
    push(errors, counts[profile] >= 15, profile + ' requires at least 15 cases');
  }
