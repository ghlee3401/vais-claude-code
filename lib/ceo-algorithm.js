 ...
 * v0.67.0 와 backward compatible: agentTeams.enabled=false 면 호출하지 않거나 무시.
 */
 ...
 * @param {string} [request.input]   - `rawText` 미지정 시 폴백. v0.66.1 backward-compat.
 *                                     (v0.65.3 문서가 `input` 키로 안내한 흔적 흡수 — 신규 호출은 `rawText` 사용 권장.)
...
function analyzeCEO(request) {
  ...
  return {
    ...
    // v2 신규 필드 (v0.67.0 backward compatible — 키 추가만)
    parallelGroup,
    dependencies,
    synthesizer,
    participants,
