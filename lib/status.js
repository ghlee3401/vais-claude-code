...
/**
 * 활성 피처 가져오기 — v3 (activeFeature) / v4 (activeFeatures[]) 호환.
 * v4 면 첫 번째 활성 피처 반환 (backward compatible).
 */
...
/**
 * v4 신규 — 다중 활성 피처 배열 반환. v3 status 면 단일 → 배열로 wrap.
 */
function getActiveFeatures() {
  const status = getStatus();
  if (Array.isArray(status.activeFeatures)) return status.activeFeatures;
  return status.activeFeature ? [status.activeFeature] : [];
}
...
/**
 * v2.0 — role 별 mandatory phase 목록.
 *
 * - cto:    plan/design/do/qa/report (sequential, mandatory)
 * - ceo:    ideation 만 mandatory (전체 PDCA 는 라우팅, mandatory 아님)
 * - cpo/cso: CEO 알고리즘이 활성화한 phase 만 (mandatory 미적용)
 * - cbo/coo: secondary — 사용자 명시 호출만, mandatory 미적용
 *
 * @param {string} role - c-level slug (ceo|cpo|cto|cso|cbo|coo)
 * @returns {string[]} mandatory phase 목록 (없으면 빈 배열)
 */
function getMandatoryPhases(role) {
  const r = (role || '').toLowerCase();
  if (r === 'cto') return ['plan', 'design', 'do', 'qa', 'report'];
  if (r === 'ceo') return ['ideation'];
  return [];
}

/**
 * v2.0 — role 이 primary (CEO 자동 라우팅 대상) 인지 판정.
 * vais.config.json > cSuite.primary 기준.
 *
 * @param {string} role
 * @returns {boolean}
 */
function isPrimaryRole(role) {
  ...
  return primary.includes((role || '').toLowerCase());
}

/**
 * v2.0 — role 이 secondary (사용자 명시 호출만) 인지 판정.
 *
 * @param {string} role
 * @returns {boolean}
 */
function isSecondaryRole(role) {
  const cfg = loadConfig();
  const secondary = cfg.cSuite?.secondary || ['cbo', 'coo'];
  return secondary.includes((role || '').toLowerCase());
}

// ============================================================
// C-Level Coexistence — Topic registry + helpers
// ============================================================
...
/**
 * Helper — phase × c-level topicPresets 조회 (v1 배열 / v2 객체 모두 지원).
 *
...
/**
 * D-Q6 인터페이스. CEO 라우팅 컨텍스트 전파용.
 * 현재는 listFeatureTopics 와 동일.
 */
...
// ============================================================
// Ideation Continuity helpers
// ============================================================

/**
 * Ideation 진행 상태 갱신 (in-progress 마킹 + 메타데이터).
 * features.{feature}.ideation = { inProgress, lastTurn, workingNotesPath, mainMdPath, lastUpdated }
 *
 * @param {string} featureName
 * @param {Object} data - { inProgress?, lastTurn?, workingNotesPath?, mainMdPath? }
 * @returns {Object|null} 갱신된 ideation 객체
 */
function setIdeationProgress(featureName, data = {}) {
  if (!validateFeatureName(featureName)) return null;
  const status = getStatus();
  if (!status.features[featureName]) {
    initFeature(featureName);
    return setIdeationProgress(featureName, data);
  }
  const feature = status.features[featureName];
  const current = feature.ideation || {};
  feature.ideation = {
    inProgress: data.inProgress !== undefined ? data.inProgress : (current.inProgress ?? false),
    lastTurn: data.lastTurn !== undefined ? data.lastTurn : (current.lastTurn ?? 0),
    workingNotesPath: data.workingNotesPath || current.workingNotesPath || `docs/${featureName}/00-ideation/working-notes.md`,
    mainMdPath: data.mainMdPath || current.mainMdPath || `docs/${featureName}/00-ideation/main.md`,
    lastUpdated: new Date().toISOString(),
  };
  saveStatus(status);
  return feature.ideation;
}
 ...
 *
 * @param {string} featureName
 * @returns {Object|null} ideation 객체 또는 null
 */
function getIdeationProgress(featureName) {
  if (!validateFeatureName(featureName)) return null;
  const status = getStatus();
  return status.features?.[featureName]?.ideation || null;
}

/**
 * 모든 in-progress ideation 목록. session-start hook 의 자동 복원용 (M0-④).
 *
 * @returns {Array<{ feature, ideation }>}
 */
function listInProgressIdeations() {
  const status = getStatus();
  const out = [];
  for (const [feature, data] of Object.entries(status.features || {})) {
    if (data?.ideation?.inProgress === true) {
      out.push({ feature, ideation: data.ideation });
    }
  }
  return out;
}
...
/**
 * Ideation 종료 (inProgress=false). 메타데이터는 보존 (이력).
 *
 * @param {string} featureName
 * @returns {Object|null}
 */
function clearIdeationProgress(featureName) {
  return setIdeationProgress(featureName, { inProgress: false });
}

// ─────────────────────────────────────────────────────────────────────────────
// Brand-First Design System — features.{feature}.brand helpers
// design-system-rethink — sets/reads the selected brand slug for ui-designer.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * 선택된 brand slug 조회. null = 미선택.
 *
 * @param {string} featureName
 * @returns {string|null}
 */
...
/**
 * brand slug 저장. design phase 진입 시 hook 또는 ui-designer 가 호출.
 *
 * @param {string} featureName
 * @param {string|null} slug - kebab-case slug, null 이면 reset
 * @returns {Object|null} 갱신된 feature 객체
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// v2 agent-teams-orchestration — Lock + Subagent Lock + Synthesis History API
// status.json v3 → v4 마이그레이션 시 lock/subagentLocks/synthesisHistory 필드 추가됨.
// 모두 advisory (강제 차단 X) — 충돌 감지 + 경고 + cleanup 가이드.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * C-Level lock 획득. 이미 점유 중이면 advisory 결과 반환 (강제 차단 X).
 */
function acquireLock(featureName, clevel, sessionId) {
  if (!validateFeatureName(featureName)) return { acquired: false, reason: 'invalid feature' };
  let status = getStatus();
  if (!status.features[featureName]) {
    initFeature(featureName);
    status = getStatus();
  ...
  const feature = status.features[featureName];
  if (feature.lock) {
    return { acquired: false, holder: feature.lock };
  }
  feature.lock = {
    clevel,
  ...
  saveStatus(status);
  return { acquired: true, lock: feature.lock };
}

function releaseLock(featureName, clevel) {
  const status = getStatus();
  const feature = status.features[featureName];
  if (!feature || !feature.lock) return false;
  if (feature.lock.clevel !== clevel) return false; // 다른 C-Level lock 은 release 못함
  feature.lock = null;
  saveStatus(status);
  return true;
}

function isStaleLock(lock, staleMinutes = 30) {
  if (!lock || !lock.acquiredAt) return false;
  const ageMs = Date.now() - new Date(lock.acquiredAt).getTime();
  return ageMs > staleMinutes * 60 * 1000;
}

/**
 * Sub-agent lock (패턴 D — features.{name}.subagentLocks[agent]).
 */
function acquireSubagentLock(featureName, agent, sessionId, branch, wtPath) {
  let status = getStatus();
  if (!status.features[featureName]) {
    initFeature(featureName);
    status = getStatus();
  ...
  const feature = status.features[featureName];
  if (!feature.subagentLocks) feature.subagentLocks = {};
  if (feature.subagentLocks[agent]) {
    return { acquired: false, holder: feature.subagentLocks[agent] };
  }
  feature.subagentLocks[agent] = {
    sessionId: sessionId || null,
  ...
  saveStatus(status);
  return { acquired: true, lock: feature.subagentLocks[agent] };
}

function releaseSubagentLock(featureName, agent) {
  const status = getStatus();
  const feature = status.features[featureName];
  if (!feature || !feature.subagentLocks?.[agent]) return false;
  delete feature.subagentLocks[agent];
  saveStatus(status);
  return true;
}

function listSubagentLocks(featureName) {
  const status = getStatus();
  return status.features[featureName]?.subagentLocks || {};
}

/**
 * Synthesis history — phase 별 합성자 추적 (감사용, v2 신규).
 */
function recordSynthesis(featureName, phase, synthesizer) {
  let status = getStatus();
  if (!status.features[featureName]) {
    initFeature(featureName);
    status = getStatus();
  ...
  const feature = status.features[featureName];
  if (!feature.synthesisHistory) feature.synthesisHistory = {};
  feature.synthesisHistory[phase] = {
    synthesizer,
    recordedAt: new Date().toISOString(),
  };
  saveStatus(status);
  return feature.synthesisHistory[phase];
}

function getSynthesisHistory(featureName) {
  const status = getStatus();
  return status.features[featureName]?.synthesisHistory || {};
}
...
module.exports = {
  ...
  setIdeationProgress,
  getIdeationProgress,
  listInProgressIdeations,
  clearIdeationProgress,
  // Brand-First Design System
  getBrand,
  setBrand,
  // v2 agent-teams-orchestration — Lock / Subagent Lock / Synthesis History
  acquireLock,
  releaseLock,
  isStaleLock,
  acquireSubagentLock,
