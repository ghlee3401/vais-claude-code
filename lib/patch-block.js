/**
 * VAIS Code - Shared body-block injection helper.
 *
 * `scripts/patch-subdoc-block.js` 와 `scripts/patch-clevel-guard.js` 의
...
function buildBlockBoundaryRegex(name, flags = '') {
  const escaped = name.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&');
...
function getVersionsFromContent(content, name) {
  const escaped = name.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&');
  const re = new RegExp(`<!--\\s*${escaped} version:\\s*(v[\\d.]+)\\s*-->`, 'g');
...
function patchFile(filePath, block, options) {
  ...
  const boundaryRe = buildBlockBoundaryRegex(name, 'g');
  ...
  if (markerRe.test(original)) {
    ...
    const existingVersions = getVersionsFromContent(original, name);
    const existingVersion = existingVersions[0];

    if (existingVersions.length === 1 && sourceVersion === existingVersion) {
      return { status: 'skip-same-version', rel, version: existingVersion };
    }

    let replacedOne = false;
    const replaced = original.replace(boundaryRe, () => {
      if (replacedOne) return '';
      replacedOne = true;
      return blockHeader + block + blockFooter;
    });
