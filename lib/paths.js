...
const CONTEXT_ROLES = new Set(['ceo', 'cpo', 'cto', 'cso', 'cbo', 'coo']);
const CONTEXT_PHASES = new Set(['ideation', 'plan', 'design', 'do', 'qa', 'report']);
const COMMON_CONTEXT_PATHS = [
  'version',
  'dependencies',
  'workflow.checkpointPolicy',
  'workflow.docPaths',
  'workflow.frontmatterMinimal',
];
const PHASE_CONTEXT_FOLDER = {
...
const PHASE_CONTEXT_PATHS = {
  ideation: ['workflow.scopeContractPolicy', 'orchestration.agentTeams'],
  plan: [
    'workflow.template',
    'workflow.scopeContractPolicy',
    'workflow.subDocPolicy',
    'workflow.cLevelCoexistencePolicy',
    'gates.defaults',
  ],
  design: [
    'designSystem',
...
// role 전용 게이트. 해당 role 로 진입할 때만 선택되고 필수로 검사된다.
const ROLE_CONTEXT_PATHS = {
  cto: { plan: ['gates.cto.plan'] },
};
const REQUIRED_CONTEXT_PATHS = {
  ideation: ['workflow.checkpointPolicy', 'workflow.docPaths'],
  plan: ['workflow.template', 'workflow.scopeContractPolicy', 'gates.defaults'],
  design: ['designSystem', 'orchestration.mcp', 'gates.defaults'],
  do: ['orchestration.agentTeams', 'workflow.subDocPolicy', 'gates.defaults'],
  qa: ['gapAnalysis', 'qa', 'gates.defaults'],
  report: ['gapAnalysis', 'gates.defaults'],
};
...
/**
 * `_` 로 시작하는 설정 주석 키(_description, _modeValues 등)를 재귀 제거한다.
 * 설정 파일 유지보수용 설명이며 phase 실행에는 값만 필요하다.
 */
...
/**
 * LLM phase 진입에 필요한 설정만 live config에서 추출한다.
 * 필수 키 누락/잘못된 인자 시 전체 config를 반환해 기존 동작을 보존한다.
 *
 * @param {string} role
 * @param {string} phase
 * @returns {object}
 */
function buildPhaseContext(config, role, phase) {
  const normalizedRole = String(role || '').toLowerCase();
  const normalizedPhase = String(phase || '').toLowerCase();

  if (!CONTEXT_ROLES.has(normalizedRole) || !CONTEXT_PHASES.has(normalizedPhase)) {
    return {
      ...config,
      _contextView: {
        mode: 'full',
        ...
        reason: 'unsupported-role-or-phase',
      },
    };
  }

  const phasePaths = PHASE_CONTEXT_PATHS[normalizedPhase] || [];
  const rolePaths = (ROLE_CONTEXT_PATHS[normalizedRole] || {})[normalizedPhase] || [];
  const selectedPaths = [
    ...COMMON_CONTEXT_PATHS,
    ...phasePaths,
    ...rolePaths,
    ...(phasePaths.includes('gates.defaults') ? [`gates.roleOverrides.${normalizedRole}`] : []),
    `workflow.phaseArtifactMapping.${PHASE_CONTEXT_FOLDER[normalizedPhase]}`,
  ];
  const requiredPaths = [
    'dependencies',
    'workflow.checkpointPolicy',
    'workflow.docPaths',
    ...(REQUIRED_CONTEXT_PATHS[normalizedPhase] || []),
    ...rolePaths,
  ];
  const missingPaths = requiredPaths.filter(p => getNestedValue(config, p) === undefined);
  ...
  if (missingPaths.length > 0) {
    return {
      ...
      _contextView: {
        ...
        reason: 'required-config-missing',
        missingPaths,
      },
    };
  }
  ...
  for (const configPath of selectedPaths) {
    ...
    if (value !== undefined) setNestedValue(compact, configPath, stripAnnotations(value));
  }
  return compact;
}

function getPhaseContext(role, phase) {
  return buildPhaseContext(loadConfig(), role, phase);
}
