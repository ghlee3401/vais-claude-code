...
// ── v0.50 경로 규칙 ────────────────────────────────────────────────
// @see docs/01-plan/features/v050/00-migration-foundation.plan.md §2.4
...
/**
 * ideation phase 산출물 경로.
 *   docs/00-ideation/{role}_{topic}.md
 */
function ideationPath(role, topic) {
  validateComponent(role, 'role');
  validateComponent(topic, 'topic');
  return safePath(PROJECT_DIR, path.join('docs', '00-ideation', `${role}_${topic}.md`));
}
