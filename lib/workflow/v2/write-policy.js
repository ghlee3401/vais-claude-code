...
const READ_ONLY_COMMANDS = [
  ...
  /^\s*git\s+(?:status|diff|log|show|rev-parse|remote\s+get-url)\b/,
];
...
const CHECK_COMMANDS = [
  ...
  /^\s*node\s+--test\b/,
  /^\s*npx\s+eslint\b/,
  /^\s*node\s+scripts\/(?:vais-validate-plugin|doc-validator|template-validator)\.js\b/,
];
...
function runtimeCommandTail(command, authorization) {
  const trimmed = String(command || '').trim();
  const configured = (authorization?.allowedCommands || [])
    .map(value => String(value).trim())
    .find(prefix => /vais-workflow-v2\.js"?$/.test(prefix) &&
...
function authorizeCommand(command, authorization) {
  const value = String(command || '');
  ...
  const runtimeTail = runtimeCommandTail(value, authorization);
