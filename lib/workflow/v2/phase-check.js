...
// Kinds whose Plan is one line and whose Design is a list of options (stage-* and ui) keep
// their canonical content elsewhere, so REQ/TC traceability does not apply to them.
function usesShortTemplates(kind) {
  return kind?.planTemplate === 'one-line' || ['options', 'options-screens'].includes(kind?.designTemplate);
}
...
const STAGE_TEMPLATE_SECTIONS = Object.freeze({
  plan: [
    ...
    ['kind', /\bkind\b/i],
    ['stage-or-target', /단계|대상\s*화면|target/i],
  ],
  design: [
...
function optionCountFindings(item, phase, content) {
  if (phase !== 'design' || !['options', 'options-screens'].includes(kindOf(item)?.designTemplate)) return [];
...
function traceabilityFindings(projectRoot, item, phase, content) {
  ...
  if (!['design', 'review'].includes(phase) || usesShortTemplates(kindOf(item))) return findings;
...
module.exports = { REQUIRED_SECTIONS, STAGE_TEMPLATE_SECTIONS, OPTION_LIMITS, usesShortTemplates, requiredSectionsFor, optionCountFindings,
  optionScreenshotFindings, phaseDocumentPath, extractTraceIdentifiers, compareIdentifierSets, traceabilityFindings, inspectPhaseDocument, checkPhaseDocument };
