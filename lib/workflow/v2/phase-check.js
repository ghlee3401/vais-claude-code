...
function requiredSectionsFor(item, phase) {
  ...
  if (!isStageKind(kind)) return REQUIRED_SECTIONS[phase] || [];
...
function optionCountFindings(item, phase, content) {
  if (phase !== 'design' || !isStageKind(kindOf(item))) return [];
...
function traceabilityFindings(projectRoot, item, phase, content) {
  ...
  if (!['design', 'review'].includes(phase) || isStageKind(kindOf(item))) return findings;
...
module.exports = { REQUIRED_SECTIONS, STAGE_TEMPLATE_SECTIONS, OPTION_LIMITS, requiredSectionsFor, optionCountFindings,
  phaseDocumentPath, extractTraceIdentifiers, compareIdentifierSets, traceabilityFindings, inspectPhaseDocument, checkPhaseDocument };
