...
// The output contract's numbers, in the words the Agent prompt shows the specialist. Built from
// the contract itself so the guidance can never drift from what the validator enforces.
function describeHandoffLimits(outputContract) {
  const properties = outputContract?.properties || {};
  const cap = name => properties[name] ? `${properties[name].maxItems ?? '∞'}개 × ${properties[name].items?.maxLength ?? '∞'}자` : '없음';
  const behavior = properties.behavior?.properties?.inputs;
  const lines = [
    `handoff 출력 계약 (직렬화 UTF-8 최대 ${outputContract['x-vais-max-serialized-bytes'] ?? '∞'}B, 목표 ${outputContract['x-vais-target-serialized-bytes'] ?? '∞'}B; 글자는 한국어도 1로 세되 각 한도의 80% 안에 둔다):`,
    `- judgment ≤ ${properties.judgment?.maxLength ?? '∞'}자 · decisions ${cap('decisions')} · behavior.inputs/outputs/errors 각 ${behavior ? `${behavior.maxItems}개 × ${behavior.items?.maxLength}자` : '없음'}`,
    `- evidence ${cap('evidence')} (짧은 상대 경로·receipt ID) · risks ${cap('risks')} · unverified ${cap('unverified')} · recommendedChecks ${cap('recommendedChecks')} · affectedRequirements ≤ ${properties.affectedRequirements?.maxItems ?? '∞'}개`,
    '- 계약에 없는 키는 금지다. `files` 는 실제로 쓴 파일 목록이므로 codeWrite 가 false 인(읽기 전용) assignment 에서는 넣지 않는다.',
  ];
  return lines.join('\n');
}
