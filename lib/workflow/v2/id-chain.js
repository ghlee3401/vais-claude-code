...
function stageDocumentCheck(projectRoot, item) {
  ...
  const findings = document
    ? validateStageDocument(projectRoot, stage, document.parsed, loadChainIndex(projectRoot), { rawText: document.raw })
  ...
  return {
    ...
    summary: findings.length === 0
      ? `${stage.title} ${document.parsed.items.length}개 항목이 형식·부모·산출물·예산 검사를 통과`
...
function confirmUnchanged(projectRoot, itemId, parentId) {
  ...
  if (!parent || !(item.parents || []).includes(parentId)) throw new Error(`${parentId} is not a parent of ${itemId}`);
