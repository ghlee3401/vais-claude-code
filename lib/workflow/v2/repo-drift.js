...
function workItemDocumentPrefix(workItem) {
  const topFeature = String(workItem?.primaryFeature || '').split('/')[0];
  const folder = String(workItem?.id || '').replace(/^WI-/, '');
  if (!topFeature || !folder) return null;
  return `docs/work-items/${topFeature}/${folder}/`;
}

// The runtime writes evidence, transaction receipts, drafts, and handoff files under
// the Work item's own document tree while a phase is in progress. Those files are the
// runtime's own bookkeeping, not repository drift. Only the canonical phase `main.md`
// files stay observable so an external edit to an approved document is still routed.
function isRuntimeOwnedPath(file, workItem) {
  const prefix = workItemDocumentPrefix(workItem);
  if (!prefix || !String(file).startsWith(prefix)) return false;
  return path.posix.basename(String(file)) !== 'main.md';
}

function filterExternalDrift(paths, workItem) {
  return (paths || []).filter(file => !isRuntimeOwnedPath(file, workItem));
}
...
module.exports = {
  changedFiles, captureRepoSnapshot, diffSnapshots, classifyDrift, scopedSnapshotDigest,
  workItemDocumentPrefix, isRuntimeOwnedPath, filterExternalDrift,
};
