...
const STAGE_PHASE_BUDGETS = Object.freeze({ plan: 2048, design: 6144, do: 2560, review: 4096, report: 2560 });
...
function evaluateDocumentBudget(projectRootOrInput, item, phase) {
  ...
  const limit = isStageKind(input.kind) ? STAGE_PHASE_BUDGETS[input.phase] : DOCUMENT_BUDGETS[input.scale]?.[input.phase];
  const findings = [];
