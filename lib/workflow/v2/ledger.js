...
// `/vais 기록 <종류> <내용>` accepts Korean or English kind names; milestones stay runtime-only.
const KIND_ALIASES = Object.freeze({
  결정: 'decision', decision: 'decision',
  피드백: 'feedback', feedback: 'feedback',
  취향: 'preference', preference: 'preference',
  부채: 'debt', debt: 'debt',
  리스크: 'risk', 위험: 'risk', risk: 'risk',
  메모: 'note', note: 'note',
});
const USER_KINDS = Object.freeze(['decision', 'feedback', 'preference', 'debt', 'risk', 'note']);
...
function entriesForTransition(projectRoot, context) {
  ...
  switch (event) {
    ...
    case EVENTS.REPORT_VALIDATED:
      push('milestone', kindOf(item)?.designTemplate === 'bugfix' ? `버그 해결 — ${item.title}` : `작업 완료 — ${item.title}`, 'Report 가 확정되어 동결됐다', [reportPath(item)]);
      for (const limitation of payload.limitations || []) push('debt', clip(limitation), `${item.id} Report 의 잔여 제한`, [reportPath(item)]);
      for (const debt of payload.resolvedDebts || []) push('note', `부채 해소: ${clip(debt, 360)}`, `${item.id} 가 이 부채를 해소했다`, [reportPath(item)]);
      break;
    default:
      break;
  }
