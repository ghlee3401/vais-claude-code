...
const { readConfigFile, loadWorkflowConfig, loadDocumentBudgets, resolveMode, harnessSwitchOff } = require('./config');
...
// vais.config.json > documentBudgets: a bad cell is ignored by the runtime (fail-closed to the
// default), so doctor is the only place the author hears about it.
function checkDocumentBudgets(root) {
  const loaded = loadDocumentBudgets(root);
  if (loaded.error) return check('document-budgets', 'warn', 'vais.config.json 을 읽지 못해 기본 예산을 쓴다', 'config-parse 항목을 먼저 고친다');
  if (loaded.ignored.length > 0) {
    return check('document-budgets', 'warn', `무시된 예산 설정: ${loaded.ignored.join(', ')}`,
      'documentBudgets.{compact|standard|extended|stage}.{plan|design|do|review|report} 에 양의 정수(byte)만 둔다');
  }
  if (!loaded.present || loaded.overridden === 0) return check('document-budgets', 'pass', '문서 예산 기본값 사용');
  return check('document-budgets', 'pass', `문서 예산 덮어쓴 칸 ${loaded.overridden}개`);
}
