...
const { normalizeKind } = require('./ledger');

const MANAGED_PREFIX = /^\s*\/vais(?:\s+|$)/i;
// User commands (docs/harness/design.md §7). Read commands answer from state; write commands
// (save · revert · ledger) need the user's own confirmation sentence, which the prompt hook
// turns into a session token the CLI checks.
const COMMAND_PATTERNS = Object.freeze([
  ['explain', /^(?:설명|explain)\s+(.+)$/i, false, match => ({ target: match[1].trim() })],
  // `commit` is an alias only as the bare word or `commit 확인|confirm`; "commit 기능 만들어" stays a request.
  ['save-confirm', /^(?:저장|save|commit)\s*(?:확인|confirm)(?:\s*[:：]\s*([\s\S]+))?$/i, true, match => ({ message: match[1] ? match[1].trim() : null })],
  ['save', /^(?:(?:저장|save)(?:\s+([\s\S]+))?|commit)$/i, false, match => ({ message: match[1] ? match[1].trim() : null })],
  ['revert-confirm', /^(?:되돌리기|revert)\s*확인\s*[:：]\s*(\S+)$/i, true, match => ({ target: match[1] })],
  ['revert', /^(?:되돌리기|revert)\s+(\S+)$/i, false, match => ({ target: match[1] })],
  ['propose', /^(?:제안|propose|suggest)\s*[.!?]?$/i, false, () => ({})],
  // `/vais diagram <요청>` opens the diagram skill with the stand-alone output folder in scope.
  // State-neutral like `save`: it never moves the Work item.
  ['diagram', /^(?:diagram|다이어그램)\s+([\s\S]+)$/i, true, match => ({ request: match[1].trim() })],
  ['ledger-list', /^(?:기록\s*보기|기록\s*조회|notes)(?:\s+(\S+))?\s*$/i, false, match => ({ kind: match[1] ? normalizeKind(match[1]) : null, rawKind: match[1] || null })],
  ['ledger-add', /^(?:기록|note)\s+(\S+)\s+([\s\S]+)$/i, true, match => ({ kind: normalizeKind(match[1]), rawKind: match[1], text: match[2].trim() })],
  // `/vais 정리: 범위 X` proposes the scope migration (old flat documents and per-stage Feature
  // folders → one scope); `/vais 정리 확인` runs it with the user's sentence as the token.
  ['migrate-confirm', /^(?:정리|migrate)\s*(?:확인|confirm)(?:\s*[:：]\s*(?:범위|scope)\s+([a-z0-9][a-z0-9-]{1,47}))?\s*$/i, true, match => ({ scope: match[1] ? match[1].toLowerCase() : null })],
  ['migrate', /^(?:정리|migrate)\s*[:：]\s*(?:범위|scope)\s+([a-z0-9][a-z0-9-]{1,47})\s*$/i, false, match => ({ scope: match[1].toLowerCase() })],
]);

function routeCommand(text) {
  for (const [action, pattern, mutationAllowed, extract] of COMMAND_PATTERNS) {
    const match = text.match(pattern);
    if (!match) continue;
    const fields = extract(match);
    // "기록 화면 만들기" is a request, not a ledger note: an unknown kind falls through.
    if (action === 'ledger-add' && !fields.kind) continue;
    return { managed: true, action, mutationAllowed, text, ...fields };
  }
  return null;
}
...
// `이름: <kebab>` names the Feature; anything after the name is plain request text and never
// joins the slug (harness-guidance-limits: "이름: a-b 추려서 Plan" once became "a-b-plan").
const NAME_FEATURE = /^(?:이름|name|범위|scope)\s*[:：]\s*([A-Za-z0-9][A-Za-z0-9-]{1,47})(?:\s+[\s\S]*)?$/i;
// `범위: <kebab>` may also sit inside the sentence (`/vais 새 제품: po-report 범위: member-management`):
// the scope names the Feature of a stage Work item (harness-scope-sections REQ-001).
const SCOPE_IN_TEXT = /(?:^|\s)(?:범위|scope)\s*[:：]\s*([A-Za-z0-9][A-Za-z0-9-]{1,47})(?=\s|$)/i;

function scopeNameIn(text) {
  const value = String(text || '').trim();
  const match = value.match(NAME_FEATURE) || value.match(SCOPE_IN_TEXT);
  return match ? match[1].toLowerCase() : null;
}
// `/vais 변경 없음 확인: F-003 ← REQ-002` — the user, not the AI, declares that a changed
// parent does not affect a child. The runtime records the pair before the CLI may use it.
const STAGE_CONFIRM_UNCHANGED = /^(?:변경\s*없음\s*확인|unchanged)\s*[:：]\s*([A-Z]{1,4}-\d{3})\s*(?:←|<-)\s*([A-Z]{1,4}-\d{3})\s*$/i;
...
const AMBIGUOUS_POSITIVE = /^(?:좋아|좋네|괜착네|괜찮네|오케이|ok|okay|그래)(?:\s*[.!])?$/i;
// Screen check (docs/harness/design.md §5): only an explicit "done" moves to Review; any other
// sentence at do/waiting-user is a revision request that travels verbatim into the ledger.
const SCREEN_CONFIRM = /^(?:확인|됐다|됐어|됐습니다|이대로\s*(?:좋아|진행)|confirm|confirmed|done)(?:\s*[.!])?$/i;
// `/vais 2번` picks a Design option before approval.
const OPTION_CHOICE = /^(?:안\s*)?(\d{1,2})\s*번(?:\s*(?:안|으로|이요|이야|선택|고를게|할게))?\s*[.!]?$/i;
...
function routePrompt(prompt, currentWorkItem) {
  ...
  const unchanged = text.match(STAGE_CONFIRM_UNCHANGED);
  if (unchanged) {
    return {
      managed: true, action: 'stage-confirm-unchanged', mutationAllowed: true, text,
      item: unchanged[1].toUpperCase(), parent: unchanged[2].toUpperCase(),
    };
  }
  if (!currentWorkItem) {
    const named = scopeNameIn(text);
    if (named) return { managed: true, action: 'name-feature', mutationAllowed: true, text, slug: named };
  }
  ...
  if (currentWorkItem?.phase === 'design' && currentWorkItem.status === 'waiting-user') {
    const choice = text.match(OPTION_CHOICE);
    if (choice) return { managed: true, action: 'choose-option', mutationAllowed: true, text, option: Number(choice[1]) };
  }
  if (currentWorkItem?.phase === 'do' && currentWorkItem.status === 'blocked' && SCREEN_CONFIRM.test(text)) {
    return { managed: true, action: 'screen-confirm', mutationAllowed: true, text };
  }
  if (currentWorkItem?.phase === 'do' && currentWorkItem.status === 'waiting-user') {
    if (SCREEN_CONFIRM.test(text)) return { managed: true, action: 'screen-confirm', mutationAllowed: true, text };
    if (EXPLICIT_PENDING_REQUEST.test(text)) return { managed: true, action: 'queue-pending', mutationAllowed: true, text };
    return { managed: true, action: 'screen-revise', mutationAllowed: true, text };
  }
  if (!currentWorkItem) {
