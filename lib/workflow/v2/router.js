...
const SAFE_APPROVAL_SUFFIXES = Object.freeze([
  ...
  /^이\s*격리\s*벤치마크\s*실행의\s*사용자\s*승인을\s*명시적으로\s*부여합니다$/i,
  /^report\s*정본과\s*feature\/work-item\s*인덱스만\s*완성하고\s*구현\s*코드는\s*바꾸지\s*마(?:세요|십시오)?$/i,
  /^승인된\s*plan을\s*기준으로\s*design\s*gate까지만\s*진행하고\s*승인을\s*요청해$/i,
  /^승인된\s*범위로\s*구현하고\s*do\s*readiness\s*gate까지만\s*진행해$/i,
  /^승인된\s*범위로\s*구현하고\s*독립\s*qa\s*review\s*결과와\s*최종\s*승인\s*요청까지\s*자동으로\s*진행해$/i,
  /^ai\s*qa\s*pass를\s*확인했습니다$/i,
  /^report를\s*확정하고\s*완료\s*상태를\s*보여줘$/i,
]);
