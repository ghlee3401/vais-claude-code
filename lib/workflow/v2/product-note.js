...
function renderCurrent(projectRoot, context = {}) {
  ...
  for (const stage of loadChainCatalog().stages) {
    ...
    const status = !entry ? '미작성' : staleCount > 0 ? `approved · stale ${staleCount}` : entry.status;
    lines.push(`| ${stage.order} | [${stage.title}](${path.posix.basename(stage.file)}) | ${status} | ${items.length} | ${day(entry?.approvedAt)} | ${entry?.workItem || '—'} |`);
  }
