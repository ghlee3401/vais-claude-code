...
function renderCurrent(projectRoot, context = {}) {
  ...
  lines.push(...renderScopeTable(index, stale), '');
  return `${lines.join('\n')}\n`;
}

// Per-scope roll-up (harness-scope-sections REQ-004): items of every stage grouped by their
// `## 범위:` section. Product-wide stages (design system · architecture) show as "(제품 전체)";
// items of a scoped stage approved before scopes existed show as "(범위 없음)" until `/vais 정리`.
function renderScopeTable(index, stale) {
  ...
  const groupOf = item => item.scope || (scopedStages.has(item.stage) ? '(범위 없음)' : '(제품 전체)');
  const names = [...new Set(items.map(groupOf))].sort((left, right) => left.localeCompare(right));
  const lines = ['## 범위별', '', '| 범위 | 항목 | 구현됨 | stale |', '|---|---|---|---|'];
  for (const name of names) {
    const own = items.filter(item => groupOf(item) === name);
    const staleCount = stale.filter(record => own.some(item => item.id === record.id)).length;
    lines.push(`| ${name} | ${own.length} | ${own.filter(item => item.implemented).length} | ${staleCount} |`);
  }
  return lines;
}
