...
function git(projectRoot, args, options = {}) {
  return execFileSync('git', args, {
    cwd: projectRoot, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], maxBuffer: 16 * 1024 * 1024, ...options,
  }).trim();
}
...
function changedFiles(projectRoot) {
  const output = git(projectRoot, ['status', '--porcelain', '-uall']);
  return output
    ? output.split('\n').map(line => ({ status: line.slice(0, 2).trim() || '??', file: line.slice(3).trim() })).filter(entry => !entry.file.startsWith(STATE_PREFIX))
    : [];
}
...
function commitSave(projectRoot, options = {}) {
  ...
  try {
    ...
    git(projectRoot, ['add', '-A', '--', '.', `:(exclude)${STATE_PREFIX}`]);
