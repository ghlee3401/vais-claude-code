...
const { isKnownKind, repairLimitOf, kindOf } = require('./chain-registry');

const SCREEN_REVISION_LIMIT_REASON = 'screen-revision-limit';

// Kinds whose Do ends at the screen check (docs/harness/design.md §5): the user sees the
// real screens before Review starts.
function hasScreenCheck(state) {
  return kindOf(state)?.screenCheck === true;
}

function requiresChosenOption(state) {
  return kindOf(state)?.designTemplate === 'options-screens';
}
...
const EVENTS = Object.freeze({
  ...
  USER_OPTION_CHOSEN: 'user.option.chosen',
  USER_SCREEN_CONFIRMED: 'user.screen.confirmed',
  USER_SCREEN_REVISED: 'user.screen.revised',
...
function transition(current, event, payload = {}, timestamp) {
  ...
  switch (event) {
    ...
    case EVENTS.USER_OPTION_CHOSEN: {
      ...
      if (!Number.isInteger(option) || option < 1 || option > 9) throw new Error('Option number must be an integer between 1 and 9');
    ...
    case EVENTS.USER_DESIGN_APPROVED:
      ...
      if (requiresChosenOption(state) && !state.chosenOption) throw new Error('안 번호를 먼저 고른다 (`/vais N번`) — 시안 Design 은 선택 없이 승인할 수 없다');
    ...
    case EVENTS.USER_DESIGN_REVISED:
      ...
      state.status = 'active';
      // A revised Design may carry a new option set; the earlier choice no longer applies.
    ...
    case EVENTS.DESIGN_CHECKPOINT_COMPLETED:
      requireAt(state, 'design', 'active', event);
      if ((state.qaRepairCount === 0 && !(state.screenRevisionCount > 0)) || state.approvals.design !== 'approved') {
        throw new Error('Design checkpoint is allowed only after an approved Design returns from QA or the screen check');
      }
    ...
    case EVENTS.READINESS_READY:
      ...
      if (hasScreenCheck(state)) {
        // Screen check: the user sees the real screens before Review (docs/harness/design.md §5).
        state.phase = 'do';
        state.status = 'waiting-user';
      } else {
        state.phase = 'review';
      ...
      break;
    case EVENTS.USER_SCREEN_CONFIRMED:
      if (state.phase !== 'do' || !(state.status === 'waiting-user' ||
        (state.status === 'blocked' && current.blockReason === SCREEN_REVISION_LIMIT_REASON))) {
        throw new Error(`${event} is not allowed from ${state.phase}/${state.status}`);
      }
      state.phase = 'review';
      state.status = 'active';
    ...
    case EVENTS.USER_SCREEN_REVISED:
      requireAt(state, 'do', 'waiting-user', event);
      if ((state.screenRevisionCount || 0) >= repairLimitOf(state)) {
        state.status = 'blocked';
        state.blockReason = SCREEN_REVISION_LIMIT_REASON;
      } else {
        state.screenRevisionCount = (state.screenRevisionCount || 0) + 1;
        state.phase = 'design';
        state.status = 'active';
      }
      break;
...
module.exports = {
  ...
  TERMINAL_STATUSES,
  SCREEN_REVISION_LIMIT_REASON,
  EVENTS,
  hasScreenCheck,
  requiresChosenOption,
  assertState,
