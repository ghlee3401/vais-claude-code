...
const TOOL_DEFINITIONS = Object.freeze({
  ...
  // Built-in: evaluated inside the phase transaction for stage kinds (lib/workflow/v2/id-chain.js).
  'stage-document': { builtin: true },
  // Built-in: screenshots of the app for screen-check kinds (lib/workflow/v2/screen-capture.js).
  'screen-capture': { builtin: true },
  // Built-in: feature · bug Designs against the chain (lib/workflow/v2/citation.js).
  'implementation-document': { builtin: true },
  // Reserved seat for a pixel comparison adapter (docs/harness/design.md §8); not runnable yet.
  'screenshot-compare': { builtin: true, reserved: true },
});

function isBuiltinCheck(check) {
  return Boolean(TOOL_DEFINITIONS[check]?.builtin);
}
...
function runCheck(check, options = {}) {
  ...
  if (definition.builtin) throw new Error(`Built-in check ${check} runs inside the phase transaction, not as a Tool`);
...
module.exports = { TOOL_DEFINITIONS, isBuiltinCheck, resolveDefinition, defaultExecutor, hasEvidenceArtifact, runCheck };
