...
function builtinChecks(projectRoot, item) {
  return isStageKind(kindOf(item)) ? [assertContract('checkResult', stageDocumentCheck(projectRoot, item))] : [];
}
...
function runPhaseTransaction(projectRoot, input) {
  ...
  try {
    ...
    if (phase === 'plan') {
      ...
      const builtin = builtinChecks(root, item);
