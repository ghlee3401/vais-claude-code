...
const { stageDocumentCheck, assertStageEntry, approveStage, indexPath, loadChainIndex, computeStale, scopeOfWorkItem } = require('./id-chain');
const citation = require('./citation');
const { withApp } = require('./app-runner');
...
// Report text limits (characters). The prompt hook shows these same numbers in the Report
// guidance, so the author meets them on the first try (harness-guidance-limits, 4.2.1).
const REPORT_LIMITS = Object.freeze({ outcome: 700, limitation: 300 });
const REPORT_OUTCOME_LIMIT = REPORT_LIMITS.outcome;

// A frozen Report cannot be repaired, so over-long text is refused, never truncated.
function assertReportInputs(input) {
  if (input.outcome !== undefined && String(input.outcome).length > REPORT_LIMITS.outcome) {
    const error = new Error(`Report --outcome 은 ${REPORT_LIMITS.outcome}자 이내여야 한다 (현재 ${String(input.outcome).length}자). 세부는 Review·Do 문서에 있으므로 요약만 적는다`);
    error.code = 'REPORT_OUTCOME_TOO_LONG';
    throw error;
  }
  (input.limitations || []).forEach((value, index) => {
    if (String(value).length > REPORT_LIMITS.limitation) {
      const error = new Error(`Report --limitation ${index + 1}번째는 ${REPORT_LIMITS.limitation}자 이내여야 한다 (현재 ${String(value).length}자). 한 줄로 줄이거나 둘로 나눈다`);
      error.code = 'REPORT_LIMITATION_TOO_LONG';
      throw error;
    }
  });
  return input;
...
function reportBody(projectRoot, item, input) {
  ...
  return [
    ...
    '## 잔여 제한', '',
    ...((input.limitations || []).length ? input.limitations.map(value => `- ${String(value)}`) : ['- None recorded.']), '',
    '## 정본 링크', '',
...
function builtinChecks(projectRoot, item, phase = 'do') {
  ...
  if (isStageKind(kind)) checks.push(assertContract('checkResult', stageDocumentCheck(projectRoot, item)));
  if (citation.isImplementationKind(kind)) {
    checks.push(assertContract('checkResult', citation.implementationDocumentCheck(projectRoot, item)));
    // A feature that cites or declares screens photographs them as Review evidence (no screen check stop).
    if (phase === 'do' && citation.touchesScreens(citation.readDesignCitations(projectRoot, item))) {
      checks.push(assertContract('checkResult', screenCaptureCheck(projectRoot, item)));
    }
  }
...
function screenCaptureCheck(projectRoot, item) {
  ...
  const target = ui.run ? ui.run.url : (ui.url || (appRoot ? path.join(appRoot, ui.entry) : null));
  // ui.run starts the app around the capture; without it the target is a static file or a live URL.
  const runner = ui.run ? fn => withApp(projectRoot, ui.run, fn) : fn => fn(target);
  if (!target) {
    findings.push('vais.config.json > ui.appRoot(+entry), ui.url 또는 ui.run 이 없어 화면을 찍을 수 없다');
  } else {
    try {
      const shot = runner(url => capture(url, roundDir));
...
function runPhaseTransaction(projectRoot, input) {
  ...
  try {
    ...
    if (phase === 'design') {
      const kind = kindOf(item);
      if (citation.isImplementationKind(kind)) {
        // Stale gate (docs/harness/design.md §3): no implementation Design while any item is stale.
        const stale = computeStale(loadChainIndex(root));
        if (stale.length > 0) {
          const error = new Error(`상위 항목이 바뀌어 stale 인 항목이 있어 기능·버그 Design 을 제시할 수 없다: ${stale.map(entry => `${entry.id}←${entry.parent}`).join(', ')}. 그 단계를 재승인하거나 \`/vais 변경 없음 확인: <항목> ← <부모>\` 로 해소한 뒤 다시 제시한다`);
    ...
    if (documentFailures.length > 0) {
      // The first findings ride in the message so the reason is visible without opening the evidence file.
      const headline = documentFailures.flatMap(result => result.findings || []).slice(0, 3);
      const error = new Error(`Canonical ${phase} document preflight failed: ${documentFailures.map(result => result.check).join(', ')}${headline.length ? ` — ${headline.join(' / ')}` : ''}`);
    ...
    if (phase === 'plan') {
      ...
      if (gate.verdict === 'READY' && citation.isImplementationKind(kindOf(item))) {
        // Do is ready: the Design's new items join their canonical files as drafts (rolled back on failure).
        const parsed = citation.readDesignCitations(root, item);
        const targets = [...citation.previewAdditions(root, parsed, { workItem: item.id }).files, indexPath(root)];
        auxiliarySnapshots = targets.map(filePath => ({ filePath, previous: fs.existsSync(filePath) ? fs.readFileSync(filePath) : null }));
        citation.applyAdditions(root, item, parsed, { timestamp: input.timestamp });
      }
      ...
      const implementation = citation.isImplementationKind(kindOf(item));
      if (stage) paths.unshift(path.resolve(root, stage.file), indexPath(root));
      if (implementation) paths.unshift(indexPath(root));
      ...
      if (!gate) {
        ...
        if (stage) approveStage(root, stage, { workItem: item.id, timestamp: input.timestamp, scope: scopeOfWorkItem(item, stage) });
        // Implementation kinds: cited and declared items carry the 구현됨 stamp from now on.
        if (implementation) citation.markImplemented(root, item, citation.readDesignCitations(root, item), { timestamp: input.timestamp });
    ...
    if (phase === 'design') {
      ...
      next = store.apply(item.id, event, {
        gateResult: gate, reason: input.reason,
        ...(phase === 'report' ? {
          limitations: (input.limitations || []).map(value => String(value)),
          resolvedDebts: kindOf(item)?.designTemplate === 'bugfix' ? citation.readDesignCitations(root, item).resolvedDebts : [],
        } : {}),
      }, applyOptions);
    }
    stateApplied = true;
...
module.exports = { ACTIONS, RECEIPT_LIMITS, REPORT_OUTCOME_LIMIT, REPORT_LIMITS, assertReportInputs, reportBody, runtimeStamp, canonicalDoSpecialists, canonicalWriteScopes, canonicalToolChecks, screenCaptureCheck, exportCommittedApp,
