...
function hashRequest(rawText) {
  return crypto.createHash('sha256').update(String(rawText || ''), 'utf8').digest('hex');
}

function redactRequestSummary(rawText, maxLength = 240) {
  let summary = normalizeText(rawText);
  const fields = [];
  for (const [kind, pattern] of SECRET_REDACTIONS) {
    pattern.lastIndex = 0;
    if (!pattern.test(summary)) continue;
    pattern.lastIndex = 0;
    summary = summary.replace(pattern, `[REDACTED:${kind}]`);
    fields.push(kind);
  }
  if (summary.length > maxLength) summary = summary.slice(0, maxLength - 3).trimEnd() + '...';
  return {
    summary: summary || '[empty request]',
...
function inferCompileSignals(rawText, context = {}) {
  ...
  const signals = {
    ...
    newProduct: /\b(build|create|launch)\s+(?:an?\s+)?(?:new\s+)?(?:product|platform|service|app|ecosystem|ai assistant)\b|새 (?:제품|플랫폼|서비스|앱)|신규 (?:제품|플랫폼|서비스)|AI 비서.*만들|생태계.*구축/i.test(text),
