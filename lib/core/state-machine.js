 ...
 *   Pipeline level: CEO dynamic routing (analyzes feature context + artifacts to recommend next C-Level)
 ...
 * @deprecated Dynamic routing is now handled by CEO agent's context-based judgment.
 *             This function is kept for backward compatibility with PIPELINE_ROLES fallback.
 */
function getNextPipelineRole(currentRole) {
  const { loadConfig } = getPaths();
  const config = loadConfig();
  // Dynamic routing: order is no longer in config; fall back to PIPELINE_ROLES as reference
