...
function recordAction(type, target) {
  switch (type) {
    ...
    case 'agent_call': {
      ...
      // Detect A→B→A ping-pong cycles by counting consecutive 2-step repeats from tail
      const len = _agentCallStack.length;
      let recursionDepth = 0;
      if (len >= 3) {
        const a = _agentCallStack[len - 1];
        const b = _agentCallStack[len - 2];
        if (a !== b) {
          // Count how many consecutive [a,b] pairs repeat backwards
          for (let i = len - 3; i >= 0; i -= 2) {
            if (_agentCallStack[i] === a && i + 1 < len && _agentCallStack[i + 1] === b) {
              recursionDepth++;
            } else {
              break;
            }
...
function reset(scope, target) {
  if (scope === 'all' || scope === 'session') {
    ...
    // Also clear related file edits and errors for this feature
    for (const [key] of _fileEdits) {
      if (key.includes(target)) _fileEdits.delete(key);
    }
    for (const [key] of _errorRetries) {
      if (key.includes(target)) _errorRetries.delete(key);
    }
  }
}
...
function setThreshold(ruleId, newMax) {
  if (!LOOP_RULES[ruleId]) return;
  if (typeof newMax !== 'number' || newMax < 0 || !Number.isFinite(newMax)) {
    throw new Error(`Invalid threshold: must be non-negative finite number, got ${newMax}`);
  }
