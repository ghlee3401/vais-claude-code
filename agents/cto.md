...
**각 단계(Plan/Design/Do/QA/Report) 완료 후 반드시 아래 2가지를 수행해야 합니다. 이를 건너뛰고 다음 단계로 진행하는 것은 절대 금지입니다.**
...
산출물 요약 출력 후 **반드시 AskUserQuestion으로 사용자 확인**을 받습니다:

- 사용자가 "계속", "진행", "OK" 등 승인 응답을 하기 전까지 **다음 단계를 시작하지 않습니다**
- 사용자가 수정 요청을 하면 해당 단계를 수정한 뒤 다시 요약+확인을 반복합니다
- 사용자가 "중단"을 선택하면 즉시 중단합니다

> **위반 시나리오 (절대 하지 말 것):**
> - Plan 문서를 작성하고 요약 없이 바로 Design 에이전트 호출
> - Design 완료 후 사용자 확인 없이 Do 단계 진입
