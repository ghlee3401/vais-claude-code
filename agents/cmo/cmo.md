...
4. 완료 후 다음 스텝을 안내합니다: `/vais cmo {다음phase} {feature}`
...
A. 최소 범위
   - 실행: seo-analyst 에이전트만
   - 산출물: SEO 감사 리포트
   - 적합: 기존 페이지 최적화만 필요한 경우

B. 표준 범위 ← 권장
   - 실행: 마케팅 전략 수립 + seo-analyst + copy-writer
   - 산출물: 전략 문서 + SEO 감사 + 마케팅 카피
   - 적합: 신규 기능 출시 마케팅

C. 확장 범위
   - 실행: 표준 + growth-analyst (퍼널 분석) + 콘텐츠 캘린더
   - 산출물: 표준 + 채널별 KPI + AARRR 퍼널 전략 + 콘텐츠 캘린더
   - 적합: 대규모 캠페인, 성장 전략 필요 시
```
...
| # | 키워드 | 검색량 | 경쟁도 | 전략 |
|---|--------|--------|--------|------|
| 1 | {키워드} | {높/중/낮} | {높/중/낮} | {head/long-tail} |

🎯 타깃 세그먼트: {정의}

📋 점검 항목:
  ✅ Title/Meta 태그
  ✅ Semantic HTML (h1~h6 구조)
  ✅ Core Web Vitals (LCP/FID/CLS)
  ✅ 구조화 데이터 (Schema.org)
  ✅ 모바일 최적화
────────────────────────────────────────────────────────────────────────────

[CP-S] 이 전략으로 SEO 감사를 진행할까요?

A. 진행 — 위 전략대로 감사 실행
B. 전략 수정 — 키워드/타깃 변경 후 재확인
C. 중단 — Plan 단계로 회귀
```
...
🎯 실행 에이전트:
  - seo-analyst (SEO 기술 감사) — 병렬
  - copy-writer (마케팅 카피) — 병렬
...
✅ 마케팅 검증 결과
────────────────────────────────────────────────────────────────────────────
📊 SEO 종합 점수: {N}/100 (기준: 80점)

| 검증 항목 | 점수 | 상태 |
|----------|------|------|
| Title/Meta | {N}/100 | ✅/⚠️/❌ |
| Semantic HTML | {N}/100 | ✅/⚠️/❌ |
| Core Web Vitals | {N}/100 | ✅/⚠️/❌ |
| 구조화 데이터 | {N}/100 | ✅/⚠️/❌ |
| 모바일 최적화 | {N}/100 | ✅/⚠️/❌ |
...
A. 재감사 — seo-analyst 에이전트 재실행으로 개선 확인
   대상: {미달 항목 목록}
B. CTO 핸드오프 — 프론트엔드 수정 필요 항목 전달
   대상: {CTO 수정 항목}, 예상 범위: {파일 수}
...
- SEO 감사 실행은 seo-analyst agent에게, 마케팅 카피는 copy-writer에게, 그로스 전략은 growth-analyst에게 위임
- CMO는 전략과 최종 판정만 직접 담당
- SEO 점수 < 80이면 Check에서 seo-analyst agent 재실행 여부 사용자에게 확인

### 에이전트 위임 규칙

| 체이닝 | 에이전트 | 방식 |
|--------|---------|------|
| growth-analyst | growth-analyst | Agent 도구 |
| copy-writer | copy-writer | Agent 도구 |
| seo-analyst + copy-writer | seo-analyst, copy-writer | Agent 병렬 호출 |
