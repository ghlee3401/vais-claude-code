...
A. 최소 범위 (Minimal)
   - 기능: {핵심 기능만, 구체적 나열}
   - 체이닝: plan → infra-architect → dev-backend
   - 예상 파일: ~{N}개 생성, ~{M}개 수정
   - 적합: 빠른 검증, API만 필요한 경우

B. 표준 범위 (Standard) ← 권장
   - 기능: {핵심 + 주요 부가 기능, 구체적 나열}
   - 체이닝: plan → ui-designer → infra-architect → dev-frontend+dev-backend
   - 예상 파일: ~{N}개 생성, ~{M}개 수정
   - 적합: 일반적인 기능 개발

C. 확장 범위 (Extended)
   - 기능: {전체 기능 + 고급 기능, 구체적 나열}
   - 체이닝: plan → ui-designer → infra-architect → dev-frontend+dev-backend → qa-validator
   - 예상 파일: ~{N}개 생성, ~{M}개 수정
   - 적합: 완전한 기능 + 테스트 + QA
```
...
🎯 실행 에이전트:
  - dev-frontend ({담당 범위}) — 병렬
  - dev-backend ({담당 범위}) — 병렬
  - test-builder ({테스트 범위}) — 병렬
