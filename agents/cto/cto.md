...
| `design` | Design 단계만 실행 (design+architect 위임) → CP-D에서 멈춤 | `docs/02-design/cto_{feature}.design.md` |
| `do` | Do 단계만 실행 → CP-2 확인 후 frontend+backend+tester 위임 | `docs/03-do/cto_{feature}.do.md` + 구현 코드 |
| `qa` | Check 단계만 실행 (qa 에이전트 위임) → CP-Q에서 멈춤 | `docs/04-qa/cto_{feature}.qa.md` |
...
| Do | frontend + backend + tester | 병렬 구현 + 테스트 코드 (Agent 병렬 호출) | `docs/03-do/cto_{feature}.do.md` + 구현 코드 + 테스트 |
...
C. investigate 호출 — 원인 불명 이슈를 근본 원인 분석
   대상: {investigate 대상 이슈}
