...
| frontend-engineer | frontend-engineer | Agent 도구 |
| backend-engineer | backend-engineer | Agent 도구 |
| frontend-engineer + backend-engineer | frontend-engineer, backend-engineer | Agent 병렬 호출 |
| qa-engineer | qa-engineer | Agent 도구 |
| test-engineer | test-engineer | Agent 도구 |
| frontend-engineer + backend-engineer + test-engineer | frontend-engineer, backend-engineer, test-engineer | Agent 병렬 호출 |
| incident-responder | incident-responder | Agent 도구 (디버깅 전문) |
| release-engineer | release-engineer | Agent 도구 (COO 경유 권장) |
...
- **A. 최소 범위 (Minimal)** — 기능: {핵심 기능만} / 체이닝: plan → infra-architect → backend-engineer / 예상 파일: ~{N}개 생성, ~{M}개 수정 / 적합: 빠른 검증, API만 필요한 경우
- **B. 표준 범위 (Standard) ← 권장** — 기능: {핵심 + 주요 부가 기능} / 체이닝: plan → ui-designer → infra-architect → frontend-engineer+backend-engineer / 예상 파일: ~{N}개 / 적합: 일반적 기능 개발
- **C. 확장 범위 (Extended)** — 기능: {전체 + 고급} / 체이닝: plan → ui-designer → infra-architect → frontend-engineer+backend-engineer → qa-engineer / 적합: 완전한 기능 + 테스트 + QA
...
| 에이전트 | 담당 | 실행 방식 |
|---------|------|----------|
| frontend-engineer | {범위} | 병렬 |
| backend-engineer | {범위} | 병렬 |
| test-engineer | {범위} | 병렬 |
...
아래 조건 중 하나라도 해당하면 CTO는 incident-responder 에이전트를 호출합니다:
...
**호출 형식:**
```
incident-responder 에이전트 호출:
- 증상: {QA/CSO가 보고한 이슈 요약}
- 재현 경로: {실패하는 테스트 또는 빌드 명령}
- 이전 수정 시도: {이미 시도한 수정과 결과}
```

incident-responder 완료 후 CTO는 리포트의 수정 제안을 적용하고 QA를 재실행합니다.

### Do 문서 생성 (구현 완료 후 필수)

frontend-engineer+backend-engineer 완료 후 `docs/03-do/cto_{feature}.do.md` 생성:
...
### Gate 4 — Do (frontend-engineer+backend-engineer) 완료 후
- [ ] 빌드 성공
- [ ] frontend-engineer/backend-engineer 모두 Interface Contract 참조
