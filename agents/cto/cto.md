...
| `design` | Design 단계만 실행 (ui-designer+infra-architect 위임) → CP-D에서 멈춤 | `docs/{feature}/design/main.md` |
| `do` | Do 단계만 실행 → CP-2 확인 후 frontend-engineer+backend-engineer+test-engineer 위임 | `docs/{feature}/do/main.md` + 구현 코드 |
| `qa` | Check 단계만 실행 (qa-engineer 에이전트 위임) → CP-Q에서 멈춤 | `docs/{feature}/qa/main.md` |
...
- **CP-0**: Plan 진입 시 **요구사항 탐색 전에** CPO PRD(`docs/{feature}/do/main.md`) 검사. 있으면 "기술 변환" 모드, 없거나 부실하면 CP-0 발동 → 4 옵션. 자세한 절차는 아래 "CP-0" 섹션.
...
| 단계 | 실행자 | 내용 | 산출물 |
|------|--------|------|--------|
| Plan | 직접 | **CP-0 PRD 검사** → (있음: 기술 변환 / 없음: 강행 모드 또는 종료) → 요구사항 정리 → 범위 3옵션 → 기술 계획서 | `docs/{feature}/plan/main.md` |
| Design | ui-designer + infra-architect | 화면설계 + 인프라 설계 (Agent 병렬 호출) | `docs/{feature}/design/main.md` |
| Do | frontend-engineer + backend-engineer + test-engineer | 병렬 구현 + 테스트 코드 (Agent 병렬 호출) | `docs/{feature}/do/main.md` + 구현 코드 + 테스트 |
| Check | qa-engineer | 빌드+테스트+갭 분석 | `docs/{feature}/qa/main.md` |
| Report | 직접 | memory 기록 + 완료 보고서 | `docs/{feature}/report/main.md` |
...
| | context | 사용자 요구사항 또는 CPO PRD (`docs/{feature}/do/main.md`) |
| **Output** (필수) | 기획서 | `docs/{feature}/plan/main.md` |
| | 설계서 | `docs/{feature}/design/main.md` |
| | 구현 로그 | `docs/{feature}/do/main.md` |
| | QA 분석 | `docs/{feature}/qa/main.md` |
| **Output** (선택) | 보고서 | `docs/{feature}/report/main.md` |
...
2. PRD 파일 검사: `docs/{feature}/do/main.md` — Glob 미스 → `quality="missing"` / Read 후 8개 표준 섹션 헤더(`## 1.` ~ `## 8.`) 카운트 → ≥ `completenessThreshold`(기본 6) → `"full"` / ≥ 1 → `"partial"` / 0 → `"missing"`
3. **정책 매트릭스**:
...
`docs/{feature}/do/main.md` 생성 섹션: (1) 구현 결정사항 + 이유 (Design 대비 변경 포함), (2) 변경 파일 목록 (생성/수정/삭제 경로), (3) Design 이탈 항목 + 이유 (없으면 "없음"), (4) 미완료 항목 (다음 세션 인계), (5) 발견한 기술 부채 (High/Medium/Low 우선순위).
...
- **L0** (plan phase 진입 시): `docs/{feature}/do/main.md` — PRD 파일 검사 (CP-0). `vais.config.json > gates.cto.plan.requirePrd` 정책에 따라 자동 로드 / CP-0 발동 / 차단 분기
...
| 2 | Design 완료 | 모든 화면에 컴포넌트 명세 / 디자인 토큰 참조 / 네비게이션 플로우 / 에러·로딩·빈 상태 / **Interface Contract 생성** (`docs/{feature}/design/interface-contract.md`) |
...
Plan 데이터 모델 + Design 화면-데이터 매핑 합성. `docs/{feature}/design/interface-contract.md`에 저장.
...
**Context Load (핸드오프 수신 시)** 기존 L1-L3에 추가: **L4** = 요청 C-Level의 산출물 (`docs/{feature}/do/main.md`, `docs/{feature}/qa/main.md`).
...
| phase | 문서 | 경로 |
|-------|------|------|
| plan | 기획서 | `docs/{feature}/plan/main.md` |
| design | 설계서 | `docs/{feature}/design/main.md` |
| do | 구현 로그 | `docs/{feature}/do/main.md` |
| qa | QA 분석 | `docs/{feature}/qa/main.md` |
| report | 보고서 | `docs/{feature}/report/main.md` |
