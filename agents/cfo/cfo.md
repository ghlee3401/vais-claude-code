...
| `do` | Do 단계만 실행 → CP-2 확인 후 cost-analyst/pricing-modeler 위임 | `docs/03-do/cfo_{feature}.do.md` |
...
4. 완료 후 다음 스텝을 안내합니다: `/vais cfo {다음phase} {feature}`
...
A. 최소 범위
   - 분석: ROI 계산만 (비용 vs 기대 수익)
   - 산출물: ROI 수치 + 간단 비용 분석
   - 적합: 빠른 의사결정, 내부 검토용

B. 표준 범위 ← 권장
   - 분석: ROI + pricing-modeler (가격 시뮬레이션) + 예산 계획
   - 산출물: ROI + 가격 전략 + 예산 계획 + 경쟁사 벤치마크
   - 적합: 일반적인 기능 출시 재무 분석

C. 확장 범위
   - 분석: 표준 + cost-analyst (인프라 비용 최적화) + 시나리오 분석 (낙관/기본/비관)
   - 산출물: 표준 + 인프라 비용 최적화 + 3개 시나리오 수익 예측
   - 적합: 투자 유치, 이사회 보고, 대규모 투자 결정
```
...
🎯 실행 에이전트:
  - cost-analyst (클라우드/인프라 비용 추정)
  - pricing-modeler (가격 시뮬레이션 + 수익 예측) (해당 시)
...
📈 ROI 목표: {목표}% → 현재: {실제}% — {달성 ✅ / 미달 ❌}

🔴 누락 수치 ({N}건):
  1. {항목}: {누락 사유}

📊 시나리오 분석 (확장 범위 시):
| 시나리오 | 수익 | ROI | Break-even |
|----------|------|-----|-----------|
| 낙관 | {금액} | {N}% | {N}개월 |
| 기본 | {금액} | {N}% | {N}개월 |
| 비관 | {금액} | {N}% | {N}개월 |

💡 CTO 아키텍처 변경이 필요한 경우: {있으면 설명}
──────────────────────────────────────
...
A. 보완 — 누락 수치 보완 후 재검증
   대상: {누락 항목 목록}
B. CTO 핸드오프 — 비용/아키텍처 최적화 필요 항목 전달
   대상: {CTO 변경 항목}, 예상 절감: {금액}
