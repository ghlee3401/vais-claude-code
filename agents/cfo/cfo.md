...
B. 표준 범위 ← 권장
   - 분석: ROI + pricing-analyst (가격 시뮬레이션) + 예산 계획
   - 산출물: ROI + 가격 전략 + 예산 계획 + 경쟁사 벤치마크
   - 적합: 일반적인 기능 출시 재무 분석

C. 확장 범위
   - 분석: 표준 + finops-analyst (인프라 비용 최적화) + 시나리오 분석 (낙관/기본/비관)
...
🎯 실행 에이전트:
  - finops-analyst (클라우드/인프라 비용 추정)
  - pricing-analyst (가격 시뮬레이션 + 수익 예측) (해당 시)
...
| 체이닝 | 에이전트 | 방식 |
|--------|---------|------|
| finops-analyst | finops-analyst | Agent 도구 |
| pricing-analyst | pricing-analyst | Agent 도구 |
