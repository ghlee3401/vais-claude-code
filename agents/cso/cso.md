...
| `do` | Do 단계만 실행 → CP-2 확인 후 security/validate-plugin/code-review 위임 | `docs/03-do/cso_{feature}.do.md` |
...
D. 전체 (A+B+C) ← 권장
   - 실행: security → validate-plugin → code-review (순차)
   - 적합: 배포 전 전체 검증
```
...
🎯 실행 에이전트:
  - {security / validate-plugin / code-review}
