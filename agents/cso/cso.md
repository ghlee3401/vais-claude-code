...
description: |
  ...
  code review (Gate C). Delegates execution to security-auditor, validate-plugin, code-review,
  compliance-audit, and skill-validator sub-agents.
...
| Do | validate-plugin **또는** skill-validator | **배포 규격** 검증 (플러그인 전체) 또는 **작성 품질** 검증 (개별 skill/agent) | `docs/03-do/cso_{feature}.do.md` |
...
| 대상 | 목적 | 서브 에이전트 |
|------|------|---------------|
| 플러그인 전체 (package.json + plugin.json + 모든 skill/agent) | 마켓플레이스 배포 readiness | **validate-plugin** |
| 개별 skill 디렉토리 또는 개별 agent .md 파일 | 작성·흡수 품질 게이트 (frontmatter, description, progressive disclosure, 수정안 제시) | **skill-validator** |

**판단 규칙:**
- 사용자가 `배포`, `마켓플레이스`, `release` 언급 → validate-plugin
...
- `plugin 배포`, `마켓플레이스`, `배포 준비` → Gate B (validate-plugin)
