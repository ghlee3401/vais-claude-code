...
| Do | plugin-validator **또는** skill-validator | **배포 규격** 검증 (플러그인 전체) 또는 **작성 품질** 검증 (개별 skill/agent) | `docs/03-do/cso_{feature}.do.md` |
...
| 대상 | 목적 | 서브 에이전트 |
|------|------|---------------|
| 플러그인 전체 (package.json + plugin.json + 모든 skill/agent) | 마켓플레이스 배포 readiness | **plugin-validator** |
| 개별 skill 디렉토리 또는 개별 agent .md 파일 | 작성·흡수 품질 게이트 (frontmatter, description, progressive disclosure, 수정안 제시) | **skill-validator** |

**판단 규칙:**
- 사용자가 `배포`, `마켓플레이스`, `release` 언급 → plugin-validator
- 사용자가 `스킬 검증`, `에이전트 품질`, `흡수`, `absorb`, `authoring` 언급 → skill-validator
- 모호할 경우 CP-2에서 사용자에게 AskUserQuestion으로 확인
...
B. Gate B만 — 플러그인 배포 검증
   - 실행: plugin-validator 에이전트
   - 검사: package.json, SKILL.md, 에이전트 frontmatter, 코드 안전성
   - 적합: 마켓플레이스 배포 준비 검증

C. Gate C만 — 독립 코드 리뷰
   - 실행: code-reviewer 에이전트
   - 검사: 버그 패턴, 성능 이슈, 코드 품질 (CTO QA와 독립)
   - 적합: CTO QA 이후 이중 검증

D. 전체 (A+B+C) ← 권장
   - 실행: security-auditor → plugin-validator → code-reviewer (순차)
   - 적합: 배포 전 전체 검증
```
...
🎯 실행 에이전트:
  - {security-auditor / plugin-validator / code-reviewer}
...
- 규정 준수 검증은 compliance-auditor agent에게 위임
...
| 체이닝 | 에이전트 | 방식 |
|--------|---------|------|
| compliance-auditor | compliance-auditor | Agent 도구 |
