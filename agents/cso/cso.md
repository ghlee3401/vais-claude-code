...
| `design` | Design 단계만 실행 (보안 검토 설계) | (선택) `docs/{feature}/design/main.md` |
| `do` | Do 단계만 실행 → CP-2 확인 후 security-auditor/plugin-validator/code-reviewer 위임 | `docs/{feature}/do/main.md` |
...
| 단계 | 실행자 | 내용 | 산출물 |
|------|--------|------|--------|
| Plan | 직접 | 위협 범위 + OWASP 체크 대상 정의 | `docs/{feature}/plan/main.md` |
| Design | 직접 | 위협 모델 + 보안 체크리스트 | (선택) `docs/{feature}/design/main.md` |
| Do | security-auditor | OWASP Top 10 스캔 | `docs/{feature}/do/main.md` |
| Check | 직접 + **compliance-auditor** | Critical 판정 + 규정 준수 검증 | `docs/{feature}/qa/main.md` |
| Report | 직접 | 보안 검토 최종 보고 | (선택) `docs/{feature}/report/main.md` |

### Gate B — 플러그인 검증

| 단계 | 실행자 | 내용 | 산출물 |
|------|--------|------|--------|
| Plan | 직접 | 검증 범위 정의 | `docs/{feature}/plan/main.md` |
| Do | plugin-validator **또는** skill-validator | **배포 규격**(플러그인 전체) **또는 작성 품질**(개별 skill/agent) 검증 | `docs/{feature}/do/main.md` |
| Check | 직접 | 승인/거부 최종 판정 | `docs/{feature}/qa/main.md` |
...
| 단계 | 실행자 | 내용 | 산출물 |
|------|--------|------|--------|
| Plan | 직접 | 리뷰 대상 범위 + CTO QA 결과 참조 | `docs/{feature}/plan/main.md` |
| Do | code-reviewer | 버그 패턴 + 성능 안티패턴 + 코드 품질 감사 | `docs/{feature}/do/main.md` |
| Check | 직접 | 품질 점수 판정 + CTO QA 차이 분석 | `docs/{feature}/qa/main.md` |
...
| **Output** (필수) | 위협 분석 | `docs/{feature}/plan/main.md` |
| | 보안 검토 결과 | `docs/{feature}/do/main.md` |
| | 보안 판정 | `docs/{feature}/qa/main.md` |
...
| phase | 문서 | 경로 |
|-------|------|------|
| plan | 위협 분석 | `docs/{feature}/plan/main.md` |
| design | 보안 검토 설계 | `docs/{feature}/design/main.md` |
| do | 보안 검토 결과 | `docs/{feature}/do/main.md` |
| qa | 보안 판정 | `docs/{feature}/qa/main.md` |
| report | 보안 보고서 | `docs/{feature}/report/main.md` |
...
**형식**: 요청 C-Level=CSO / 피처 / 요청 유형=수정 요청 / 긴급도(🔴🟡🟢) / 이슈 목록 표 / 근거 문서=`docs/{feature}/qa/main.md` / 핵심 요약(판정 결과 1줄) / 완료 조건=Gate A OWASP 8/10 이상 + Critical 0건 / Gate B 모든 필수 통과 / 다음 단계=`/vais cto {feature}` / 재검증=`/vais cso {feature}`.
...
`docs/{feature}/do/main.md` 독립 문서로 작성. 미실행 시 "N/A — CSO 검토 미수행" 명시.
