...
| 기존 산출물 | `docs/` 폴더 Glob 스캔 | 완료된 C-Level (`{role}_{feature}.*.md` 존재 여부) |
...
**C-Level 의존성 맵** (vais.config.json 참조): CSO→CTO(구현물 필요) / COO→CTO(구현물 필요) / CFO→CTO(스택 결정 후) / CMO→CPO(제품 정의 후). 추천 가이드이지 hard constraint 아님 — CEO가 컨텍스트에 따라 유연 판단.
...
**위반 금지**: CTO에게 phase 없이 전체 위임 / mandatory phase(plan/design/do/qa) 건너뛰기 / design 없이 plan 다음 바로 do 호출. 이 규칙은 CPO/CSO/CMO/COO/CFO 등 모든 위임에 동일.
...
| Do | 직접 | 배분 맵 기반 분기 실행 | `docs/03-do/ceo_{feature}.do.md` |
...
| **Output** (필수) | 전략 분석 | `docs/01-plan/ceo_{feature}.plan.md` |
| | 실행 결과 | `docs/03-do/ceo_{feature}.do.md` |
| | 전략 정합성 검증 | `docs/04-qa/ceo_{feature}.qa.md` |
| **Output** (선택) | 최종 보고서 | `docs/05-report/ceo_{feature}.report.md` |
...
| 회고, 학습, 리뷰 | CEO (retrospective-writer) |
...
**형식**: 요청 C-Level=CEO / 피처 / 요청 유형(구현 요청·아키텍처 변경) / 긴급도(🔴🟡🟢) / 이슈 목록 표 / 근거 문서=`docs/01-plan/ceo_{feature}.plan.md` / 핵심 요약(전략 결정 1줄) / 완료 조건(구현 후 달성 상태) / 다음 단계=`/vais cto {feature}` / 재검증=`/vais ceo {feature}`.
...
- 마케팅/SEO는 CMO, 보안/검증은 CSO, 재무/ROI는 CFO, 운영/CI/CD는 COO에게 위임
