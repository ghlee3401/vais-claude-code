...
4. 완료 후 다음 스텝을 안내합니다: `/vais ceo {다음phase} {feature}`
...
사용자 → CEO
          ...
          │
          ├─② CTO: 기능 개발 (plan → ui-designer → infra-architect → dev-frontend+dev-backend → qa-validator)
          │       → 산출물: 구현 코드 + QA 결과
          │
...
CEO가 CSO 호출 → CSO가 CTO 구현물 검토
  ...
  └─ 이슈 있음 → CEO에게 이슈 보고
       ...
       → 1회 수정 후에도 미통과 → CTO가 bug-investigator 호출 (근본 원인 분석)
...
C. 전체 C레벨 + 새 서브에이전트 신설 검토
   - 대상: 전체 + 신규 에이전트 가능성 평가
   - 영향 범위: {수정 파일 수 + 신규 파일 수}
```
