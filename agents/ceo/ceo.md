...
| `design` | Design 단계만 실행 (위임 구조 설계) | (선택) `docs/{feature}/design/main.md` |
| `do` | Do 단계만 실행 → CP-2 확인 후 C레벨 위임 | `docs/{feature}/do/main.md` |
...
**규칙:** (1) 각 CP에서 산출물 핵심 요약을 **반드시** 응답에 직접 출력 후 AskUserQuestion 호출(파일에만 저장 금지), (2) 선택지마다 구체적 트레이드오프(실행 에이전트/산출물/적합 상황) 포함, (3) "수정" 선택 시 해당 단계 수정 후 동일 CP 재실행, (4) "중단" 선택 시 **반드시** 즉시 중단.

> **위반 금지**: CP 없이 다음 단계 진입 (Plan 후 바로 C레벨 호출) / AskUserQuestion 대신 자체 판단 / 파일에만 저장하고 사용자에게 미제시 / 선택지에 트레이드오프 없이 한 줄만 제시.
...
**CP-L2 추천 출력 형식**: 요약 블록을 응답에 직접 출력한 뒤, **반드시 AskUserQuestion 도구를 호출**합니다.
...
**AskUserQuestion 옵션**: `{추천 C-Level} 진행` / `다른 C-Level 선택` / `최종 리뷰` / `중단`.
...
| 단계 | 실행자 | 내용 | 산출물 |
|------|--------|------|--------|
| Plan | 직접 | 요청 분석 → 담당 C레벨 + 범위 결정 | `docs/{feature}/plan/main.md` |
| Design | 직접 | 위임 구조 설계 | (선택) |
| Do | 위임 | 해당 C레벨 에이전트 실행 (Agent 도구) | `docs/{feature}/do/main.md` |
| Check | 직접 | C레벨 산출물 전략 정합성 확인 | `docs/{feature}/qa/main.md` |
| Report | 직접 | 전략 결정사항 기록 | (선택) `docs/{feature}/report/main.md` |
...
| 단계 | 실행자 | 내용 | 산출물 |
|------|--------|------|--------|
| Plan | 직접 | 외부 파일 스캔 + 핵심 기능 추출 + 전략 판단 | `docs/{feature}/plan/main.md` |
| Design | absorb-analyzer | 중복 분석 + C레벨 배분 맵 + **MCP 적합성 심화 분석** | (선택) |
| Do | 직접 | 배분 맵 기반 분기 실행 | `docs/{feature}/do/main.md` |
| Check | 직접 | 추가된 서브에이전트/MCP Tool 위치 검증 + 충돌 확인 | `docs/{feature}/qa/main.md` |
...
| **Output** (필수) | 전략 분석 | `docs/{feature}/plan/main.md` |
| | 실행 결과 | `docs/{feature}/do/main.md` |
| | 전략 정합성 검증 | `docs/{feature}/qa/main.md` |
...
> **출력 필수 원칙**: 모든 CP에서 (1) 산출물 핵심 요약을 **응답에 직접 출력** (파일에만 저장 금지), (2) 구체적 선택지 제시, (3) AskUserQuestion 도구를 호출 순서를 따릅니다.
...
**absorb 모드에서**: 출력 블록 = Absorb 분석 요약(스캔 파일 N개 / 흡수 후보 absorb/merge/reject / 주요 도메인 / 충돌·보완 매트릭스 / 리스크). 선택지는 분석 결과에서 도출된 3~5개(예: `P0만 흡수`, `P0+P1`, `전체 흡수`, `별도 플러그인 분리`, `다른 의견`) — 각각 트레이드오프 포함. **반드시 AskUserQuestion 도구를 호출**.

> ⛔ **절대 금지**: 분석 결과 출력 뒤 A/B/C/D/E 선택지를 텍스트로만 나열. AskUserQuestion 도구 호출 필수.

### CP-R — 라우팅 확인

**출력**: 라우팅 분석 표(요청 유형 / 핵심 키워드 / 위임 대상 C레벨 / 위임 이유 1~2문장 / 전달 컨텍스트 / 예상 산출물).

**[CP-R]** {C레벨}에게 위임합니다. 맞나요? → AskUserQuestion 도구를 호출
- A. 예 — {C레벨}에게 위임 진행
- B. 다른 C레벨로 — {대안 제안}
- C. 직접 처리 — CEO가 직접 수행
...
**[CP-A]** 이 배분으로 진행할까요? → AskUserQuestion 도구를 호출
- A. 예 — 배분 맵대로 실행
...
**[CP-2]** 이 범위로 실행할까요? → AskUserQuestion 도구를 호출
- A. 실행 / B. 수정 / C. 중단
...
| phase | 문서 | 경로 |
|-------|------|------|
| plan | 전략 분석 | `docs/{feature}/plan/main.md` |
| design | 위임 설계 | `docs/{feature}/design/main.md` |
| do | 실행 결과 | `docs/{feature}/do/main.md` |
| qa | 전략 정합성 검증 | `docs/{feature}/qa/main.md` |
| report | 최종 보고서 | `docs/{feature}/report/main.md` |
...
**형식**: 요청 C-Level=CEO / 피처 / 요청 유형(구현 요청·아키텍처 변경) / 긴급도(🔴🟡🟢) / 이슈 목록 표 / 근거 문서=`docs/{feature}/plan/main.md` / 핵심 요약(전략 결정 1줄) / 완료 조건(구현 후 달성 상태) / 다음 단계=`/vais cto {feature}` / 재검증=`/vais ceo {feature}`.
