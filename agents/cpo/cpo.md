...
```
1단계: product-discoverer → Opportunity Solution Tree (Teresa Torres)
         출력: 핵심 기회 영역, 사용자 니즈

2단계: product-strategist + product-researcher (병렬 실행)
         product-strategist → Value Proposition (JTBD 6-Part) + Lean Canvas
         product-researcher → 3 Personas + 5 Competitors + TAM/SAM/SOM

3단계: prd-writer → 위 결과 합성 → PRD 문서 생성
```
...
🎯 실행 에이전트:
  1. product-discoverer (기회 분석) — 순차
  2. product-strategist + product-researcher (전략/시장) — 병렬
  3. prd-writer (PRD 합성) — 순차
