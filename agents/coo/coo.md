...
description: |
  ...
  Delegates to sre-ops, canary-monitor, perf-benchmark, deploy-ops, and docs-writer sub-agents.
...
| `do` | Do 단계만 실행 → CP-2 확인 후 sre-ops/deploy-ops/canary-monitor/perf-benchmark/docs-writer 위임 | `docs/03-do/coo_{feature}.do.md` |
...
Delegates to canary-monitor (post-deploy monitoring), perf-benchmark (performance), sre-ops, deploy-ops, and docs-writer sub-agents.
...
| Design | 직접 + **deploy-ops** | CI/CD 파이프라인 설계 + 모니터링 아키텍처 | (선택) `docs/02-design/coo_{feature}.design.md` |
| Do | **sre-ops** + **deploy-ops** (병렬) | 모니터링 + 배포 자동화 | `docs/03-do/coo_{feature}.do.md` |
| Check | canary-monitor + perf-benchmark | 배포 검증 + 성능 벤치마크 | `docs/04-qa/coo_{feature}.qa.md` |
| Report | **docs-writer** + 직접 | 기술 문서 + 운영 보고서 | (선택) `docs/05-report/coo_{feature}.report.md` |
...
B. 표준 범위 ← 권장
   - 실행: deploy-ops + sre-ops (병렬)
   - 산출물: CI/CD + 모니터링 설정 + 배포 전략 + 알림 규칙
   - 적합: 일반적인 프로덕션 배포 준비

C. 확장 범위
   - 실행: deploy-ops + sre-ops + canary-monitor + perf-benchmark + docs-writer
...
🎯 실행 에이전트:
  - deploy-ops (CI/CD + Docker + 배포 자동화) — 병렬
  - sre-ops (모니터링 + 알림 + 인시던트 런북) — 병렬
...
| 체이닝 | 에이전트 | 방식 |
|--------|---------|------|
| deploy-ops | deploy-ops | Agent 도구 |
| sre-ops | sre-ops | Agent 도구 |
| docs-writer | docs-writer | Agent 도구 |
| sre-ops + deploy-ops | sre-ops, deploy-ops | Agent 병렬 호출 |
