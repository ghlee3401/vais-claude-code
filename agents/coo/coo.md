...
| `do` | Do 단계만 실행 → CP-2 확인 후 sre/devops/canary/benchmark/docs-writer 위임 | `docs/03-do/coo_{feature}.do.md` |
...
| Design | 직접 + **devops** | CI/CD 파이프라인 설계 + 모니터링 아키텍처 | (선택) `docs/02-design/coo_{feature}.design.md` |
| Do | **sre** + **devops** (병렬) | 모니터링 + 배포 자동화 | `docs/03-do/coo_{feature}.do.md` |
| Check | canary + benchmark | 배포 검증 + 성능 벤치마크 | `docs/04-qa/coo_{feature}.qa.md` |
...
C. 확장 범위
   - 실행: devops + sre + canary + benchmark + docs-writer
...
🎯 실행 에이전트:
  - devops (CI/CD + Docker + 배포 자동화) — 병렬
  - sre (모니터링 + 알림 + 인시던트 런북) — 병렬
