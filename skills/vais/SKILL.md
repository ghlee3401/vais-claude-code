...
description: >
  ...
  아이디어, research, 조사, 뭐 만들, 만들고 싶, 시작, 부터, 까지,
  ...
  frontend, backend, manager, 매니저, 현황, 히스토리, 부채, 의존성, 브리핑.
...
| `cto [feature]` | CTO 에이전트 — 기술 방향 설정 + 전체 오케스트레이션 |
| `ceo [feature]` | CEO 에이전트 — 비즈니스 전략 + 레퍼런스 흡수 지시 |
| `cmo [feature]` | CMO 에이전트 — 마케팅 전략 + SEO 감사 |
| `cso [feature]` | CSO 에이전트 — 보안 검토 + 플러그인 검증 |
| `absorb [path]` | 외부 레퍼런스 평가 및 흡수 (`/vais absorb ../bkit/`) |
| `ceo:cto [feature]` | C-Suite 체이닝 — CEO 전략 후 CTO 기술 실행 |
| `manager [질문/지시]` | ⚠️ DEPRECATED: `/vais cto` 를 사용하세요 |
| `auto [feature]` | ⚠️ DEPRECATED: `/vais cto` 를 직접 사용하세요 |
| `plan ~ qa` | 각 단계 개별 실행 |
...
1. **Deprecated 액션 처리**: `$0`이 `manager` 또는 `auto`이면:
   - "⚠️ `/vais manager`는 deprecated입니다. `/vais cto $1`로 자동 전환합니다." 출력
   - `$0 = cto`로 치환 후 계속 진행
2. Read 도구로 **`${CLAUDE_SKILL_DIR}/phases/$0.md`** 파일을 읽으세요.
3. 파일이 존재하면 그 안의 지침에 따라 실행하세요.
4. 파일이 없으면: "알 수 없는 액션: $0. `/vais help`로 사용법을 확인하세요."
