...
2. **설계 문서 수집**: plan, ia(태스크 플로우 포함), design, design-db 문서 읽기
3. **구현 코드 수집**: 해당 피처 코드 전체 읽기
4. **자동 비교**: 레지스트리의 `features[]` 각 항목 vs 코드 구현 여부. 구현된 기능은 레지스트리 `status`를 `"완료"`로 업데이트. **IA 태스크 플로우의 예외/엣지 케이스 처리 여부도 검증**
