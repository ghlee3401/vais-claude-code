...
- **Phase 명시**: `/vais cmo plan my-feature` → phase=`plan`, feature=`my-feature`
- **Phase 생략**: `/vais cmo my-feature` → phase=미지정, feature=`my-feature`
...
- **options**:
  ...
  - `현재 C-Level 다음 phase` — `/vais cmo {다음phase} {feature}`
  - `종료` — 작업 종료
...
- `현재 C-Level 다음 phase` → `skills/vais/phases/cmo.md` Read → `{다음phase}` 로 실행
