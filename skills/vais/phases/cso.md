...
name: cso
version: 2.0.0
description: CSO 에이전트 호출. 보안 검토(Gate A) + 플러그인 검증(Gate B) + 독립 코드 리뷰(Gate C). Primary — CEO 7 차원 알고리즘이 활성화한 phase 만 실행. main.md 인덱스 + sub-agent 직접 박제.
---

# CSO Phase

`${CLAUDE_PLUGIN_ROOT}/agents/cso/cso.md`를 읽고 그 안의 지침에 따라 실행하세요.
...
3. `node "${CLAUDE_PLUGIN_ROOT}/scripts/phase-context.js" cso {phase}` 결과의 `dependencies`에서 의존성 확인
