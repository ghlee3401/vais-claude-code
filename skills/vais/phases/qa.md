...
QA 시작 전, AskUserQuestion으로 검사 범위를 선택:
- 옵션: "전체 검사 (Recommended)" — 모든 Step 실행, "빌드 + Gap만" — Step 1~2만, "코드 리뷰만" — Step 5~5.5만, "보안 스캔만" — Step 3만
- 단일 선택
- auto 모드: 전체 검사
...
- **"이슈 선택 수정"** — AskUserQuestion **(multiSelect: true)**으로 개별 이슈를 복수 선택하여 수정 대상 지정. 옵션은 이슈 목록에서 심각도순으로 최대 4개 그룹화 (Critical 묶음, P0 묶음, P1 묶음, P2 묶음)
