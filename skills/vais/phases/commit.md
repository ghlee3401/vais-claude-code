...
**⚠️ 어떤 판단이든 반드시 AskUserQuestion으로 사용자에게 확인받아야 합니다. 자의적으로 스킵하지 마세요.**

AskUserQuestion으로 판단 결과를 제시:
- 버전 범프 필요 시: "이번 변경은 **{minor/patch}** 버전 범프를 권장합니다. 현재 {현재버전} → {제안버전}. 동의하시나요?"
  - 옵션: "{제안} (Recommended)", "다른 버전 지정", "버전 범프 안 함"
- 버전 범프 불필요 시: "이번 변경은 문서만 변경이므로 버전 범프가 불필요합니다. 동의하시나요?"
  - 옵션: "버전 범프 안 함 (Recommended)", "Patch로 올리기", "Minor로 올리기"
