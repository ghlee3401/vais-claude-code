...
- "어떤 피처(기능)를 기준으로 문서화할까요?" — Step 1에서 감지한 피처 목록을 AskUserQuestion **(multiSelect: true)**으로 제시. 사용자가 VAIS를 적용할 피처를 복수 선택. 옵션: 감지된 피처들 (최대 4개) + "전체 프로젝트". auto 모드: 전체 선택
...
1. **plan** → `docs/{feature}/plan/main.md`
...
2. **design** → `docs/{feature}/design/main.md`
...
3. **do (infra)** → `docs/{feature}/do/infra.md` (do sub-doc)
...
> **참고**: init은 QA 문서를 직접 생성하지 않습니다. 역설계 후 `/vais qa {feature}`로 QA를 실행하면 `docs/{feature}/qa/main.md`가 생성됩니다.
