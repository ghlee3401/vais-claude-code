> 원본: cathrynlavery/diagram-design main@9874ad7 `references/type-journey.md` (MIT, 2026-09-17 가져와 수정)

# User Journey Map — 유저 저니

**Best for:** what a person does across the stages of an experience and how it *feels* at each one. The sentiment curve is the load-bearing element — without it this is just a process diagram with extra rows, so if you can't name a sentiment for every stage, use **Flowchart** or **Timeline** instead.
...
The sentiment polyline is a **data curve**, not a connector between nodes — SKILL.md 의 연결선 규칙 1(orthogonal elbows)은 여기에 적용하지 않는다. This exemption covers the sentiment curve **only**; any other connector in a journey map (there normally are none) still follows the standard connector rules.
...
- **No sentiment curve.** If every stage feels the same, or you can't name a feeling, you're drawing a process — use **Flowchart** or **Timeline**.
