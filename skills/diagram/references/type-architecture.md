> 원본: cathrynlavery/diagram-design main@9874ad7 `references/type-architecture.md` (MIT, 2026-09-17 가져와 수정)

# Architecture — 구조도

**Best for:** system overviews, data-flow diagrams, integration maps, infra topology. VAIS 9단계(기술 구조)의 구조 안 그림이 이 유형이다.
...
**Rounded right-angle (orthogonal) connectors are MANDATORY** for all non-horizontal/vertical connections — diagonal `<line>` between off-axis nodes is a hard fail (see SKILL.md §6 connector rules). Two-bend elbow path with `r=8`:
