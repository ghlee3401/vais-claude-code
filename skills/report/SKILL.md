---
# 이 파일의 책임: /vais report·deck·brand 진입점 — 피처 문서 기반 brand 스타일 HTML 보고서/슬라이드 생성.
name: report
description: >
  Generates brand-styled, self-contained HTML reports and slide decks from a feature's
  plan/notes/review docs and git history. Anti-AI-slop design rules enforced via
  self-critique pass. Use when: /vais report {feature}, /vais deck {feature}, or
  /vais brand new is invoked. For arbitrary-topic executive reports use /vais brief.
---

# Report / Deck Generator (피처 전용)

소재가 **피처 문서로 고정**된 개발 보고 스킬이다. 피처와 무관한 주제의 임원/대외 보고는 `/vais brief {주제}` (`skills/brief/SKILL.md`).

시작 전 **반드시 `skills/report/design-rules.md` 를 Read** — 금지 패턴 8 + 타이포/차트/조판 규칙.

## 파이프라인 (report / deck 공통)

1. **소재 수집** — 대상 피처의 `docs/{feature}/` 3파일 + git log. 인자가 등록된 피처가 아니면 (오타 의심 시 비슷한 피처명 제시 후) `/vais brief {주제}` 를 안내하고 종료.
...
4. **생성** — 스켈레톤 Read (`templates/report.html` 또는 `templates/deck.html`) → 토큰을 CSS 변수에 주입 → design-rules 준수하며 콘텐츠 작성. 산출 위치: `docs/{feature}/assets/{name}.html`.
