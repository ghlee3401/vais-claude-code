...
```
/validate-plugin              # 현재 프로젝트 검증
/validate-plugin /path/to    # 특정 경로 검증
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/validate-plugin.js <대상경로>
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/validate-plugin.js <경로> --json
```
