...
**v0.20.1** · Claude Code Plugin

> 한 줄이면 기획부터 리뷰까지. AI가 6단계 개발 워크플로우를 자동으로 진행합니다.
...
AI가 기획 → 설계 → 아키텍트 → 프론트+백엔드 → 리뷰를 **자동으로** 진행합니다.
...
/vais review login            # 5. Gap 분석 + 코드 리뷰 + 보안
...
| 코드 품질을 자동으로 검증하고 싶다 | Review 단계 (Gap 분석 + OWASP 보안) |
...
| 혼합 | `/vais plan:design:architect:frontend+backend:review login` | 전체 순차+병렬 조합 |
| 자동 | `/vais auto login` | Manager가 전체 오케스트레이션 |
| 범위 | `/vais plan부터 review까지 login` | 한국어 범위 표현 자동 변환 |
...
```
Plan ──▶ [Gate 1] ──▶ Design ──▶ [Gate 2] ──▶ Architect ──▶ [Gate 3] ──▶ FE+BE ──▶ [Gate 4] ──▶ Review
                                    │
                              Interface Contract
                              (API 스펙 자동 생성)
```
...
| Gate 4 | FE+BE → Review | 빌드 성공, FE/BE 모두 Interface Contract 참조 |
...
| `/vais review {기능}` | Gap 분석 + 코드 리뷰 + 보안 |
...
| 설정 | 기본값 | 설명 |
|------|-------|------|
| `workflow.phases` | 7단계 배열 | plan → design → architect → frontend → backend → review → report |
...
├── agents/                  # 6명 에이전트 정의
├── templates/               # Plan/Design/Architect/Review/Report 템플릿
