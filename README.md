...
**v0.44.0** · Claude Code C-Suite 플러그인
...
| C-Level | 역할 | 하위 에이전트 |
|---------|------|-------------|
| **CEO** | 최상위 오케스트레이터 (Product Owner). 런칭 파이프라인 지휘, 라우팅, absorb | absorb-analyzer, retro-report |
| **CPO** | 제품 정의. PM 프레임워크 기반 PRD 생성 | pm-discovery, pm-strategy, pm-research, pm-prd, ux-researcher, data-analyst |
| **CTO** | 기술 총괄. Plan→Design→Do→QA 개발 워크플로우 지휘 | infra-architect, ui-designer, dev-frontend, dev-backend, qa-validator, test-builder, deploy-ops, db-architect, bug-investigator |
| **CSO** | 보안 검토. Gate A(보안)/B(플러그인)/C(코드리뷰). 이슈 시 CTO 수정 루프 | security-auditor, validate-plugin, code-review, compliance-audit |
| **CMO** | 마케팅 전략 + SEO 최적화 | seo-analyst, copy-writer, growth-analyst |
| **COO** | 배포/운영. CI/CD, 모니터링, 기술 문서 | canary-monitor, perf-benchmark, sre-ops, deploy-ops, docs-writer |
| **CFO** | 비용/가격 분석. ROI, 가격 전략 | cost-analyst, pricing-modeler |
