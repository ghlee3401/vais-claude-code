...
AI가 기획 → 설계 → 인프라 → 프론트+백엔드 → QA를 **자동으로** 진행합니다.
...
/vais infra login             # 3. DB 스키마 + 환경 설정
/vais fe+be login             # 4. 프론트+백엔드 (병렬)
...
| 프론트+백엔드를 빠르게 동시에 진행하고 싶다 | `fe+be` 병렬 실행 |
...
| 순차 | `/vais plan:design:infra login` | 기획 → 설계 → 인프라 순서대로 |
| 병렬 | `/vais fe+be login` | 프론트+백엔드 동시 |
| 혼합 | `/vais plan:design:infra:fe+be:qa login` | 전체 순차+병렬 조합 |
...
| Gate 2 | Design → Infra | 화면별 컴포넌트 명세, 디자인 토큰, **Interface Contract 생성** |
| Gate 3 | Infra → FE+BE | DB 스키마 일치, 마이그레이션 완료, 빌드 성공 |
...
| 설정 | 기본값 | 설명 |
|------|-------|------|
| `workflow.phases` | 7단계 배열 | plan → design → infra → fe → be → qa → report |
| `parallelGroups` | `["fe", "be"]` | 병렬 실행 그룹 |
...
├── agents/                  # 6명 에이전트 정의
├── templates/               # Plan/Design/Infra/QA/Report 템플릿
...
아닙니다. 단일 실행이나 순차 체이닝에서는 메인 Claude가 직접 처리합니다. 에이전트는 병렬 실행(`fe+be`)이나 자동 모드에서만 사용됩니다.
</details>
