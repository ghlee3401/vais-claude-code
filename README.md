...
```
 ┌─────────────────────────────────────────────────────────────────────┐
 │                                                                     │
 │   CEO ──▶ 전략 방향  │  CTO ──▶ 기획·설계·구현 오케스트레이션        │
 │   CMO ──▶ SEO 감사  │  CSO ──▶ 보안(OWASP) + 플러그인 검증          │
 │                                                                     │
 │   /vais ceo:cto:cso login   ←  C-Suite 체이닝                      │
...
```bash
/vais ceo:cto login           # CEO 전략 → CTO 실행
/vais cto:cso login           # CTO 구현 → CSO 보안 게이트
/vais ceo:cto:cso login       # 전체 C-Suite 순차 실행
```
...
v2.0 (0.23.0)부터 **6-레이어 C-Suite 플랫폼 아키텍처**로 재설계되었습니다:
...
Layer 3: C-Suite Agents        CTO / CEO / CMO / CSO / ...
...
각 레이어가 독립적으로 추가/교체 가능하여 대시보드나 MCP 서버 같은 확장도 재설계 없이 붙일 수 있습니다.
...
| 마케팅/SEO까지 AI가 자동으로 해줬으면 | `/vais cmo` — CMO가 SEO 감사까지 |
...
4개의 C-Suite 에이전트가 각자의 전문 영역을 담당합니다.
...
```
              ...
              └──┬──────────────┬───┘
         ┌───────▼───┐    ┌─────▼──────┐
         │ CMO(sonnet)│    │ CSO(sonnet)│
         │ 마케팅+SEO │    │ 보안+검증  │
         └───────────┘    └────────────┘
                     │ 위임
         ┌───────────▼──────────────────┐
         │  design / architect / FE+BE / qa  │
         │       구현 에이전트 팀              │
         └──────────────────────────────┘
```
...
| 에이전트 | 모델 | 담당 |
|---------|------|------|
| CTO | opus | Plan 직접 실행, 에이전트 오케스트레이션, Gate 판정 |
| CEO | opus | 비즈니스 전략 방향, Reference Absorption 지휘 및 최종 승인 |
| CMO | sonnet | 마케팅 컨텍스트 분석, SEO 감사 (vais-seo 통합) |
| CSO | sonnet | 보안 검토 Gate A (OWASP), 플러그인 검증 Gate B |
...
| C-Suite 순차 | `/vais ceo:cto:cso login` | CEO 전략 → CTO 구현 → CSO 보안 |
...
기술 오케스트레이션 전체를 담당합니다. 기존 `manager`의 역할을 계승합니다.
...
- CTO에게 기술 방향 위임, CMO에게 마케팅, CSO에게 보안 위임
...
마케팅 전략과 SEO 감사를 통합 실행합니다. 기존 `vais-seo` 기능이 완전히 통합되었습니다.

**실행 순서**:
1. 마케팅 컨텍스트 분석 (타깃/메시지/채널/핵심 키워드)
2. SEO 감사 (Title/Meta · Semantic HTML · Core Web Vitals · 구조화 데이터)
3. 산출물 생성
...
```bash
/vais cmo login          # 마케팅 + SEO
/vais seo login          # 동일 (레거시 호환)
```

### CSO (`/vais cso {feature}`)

두 가지 게이트를 운영합니다. 기존 `vais-validate-plugin` 기능이 Gate B로 통합되었습니다.
...
**Gate B — Plugin Validation** (`plugin 배포` / `마켓플레이스` 키워드 자동 트리거):
...
/vais validate-plugin    # 플러그인 검증 (Gate B, 레거시 호환)
```
...
```jsonl
{"ts":"...","event":"session_start","feature":"login","c_suite":["ceo","cto"]}
{"ts":"...","event":"agent_start","role":"cto","phase":"plan"}
{"ts":"...","event":"decision","role":"cto","type":"architecture","choice":"nextjs"}
{"ts":"...","event":"agent_stop","role":"cto","outcome":"success","doc":"docs/01-plan/login.md"}
```
...
### Deprecated (v2.0.0 이후)

| 구 커맨드 | 새 커맨드 |
|----------|----------|
| `/vais auto {기능}` | `/vais cto {기능}` |
| `/vais manager` | `/vais cto` |
| `/vais seo {기능}` | `/vais cmo {기능}` |
| `/vais validate-plugin` | `/vais cso {기능}` (Gate B) |
...
| `cSuite.roles` | 8개 C-Suite 역할 레지스트리 (cto/ceo/cmo/cso/cdo/cro/coo/cpo) |
...
├── agents/                  # C-Suite + 구현 에이전트
│   ├── cto.md              # CTO (오케스트레이터)
│   ├── ceo.md              # CEO (전략 + Reference Absorption)
│   ├── cmo.md              # CMO (마케팅 + SEO)
│   ├── cso.md              # CSO (보안 + 플러그인 검증)
...
│   └── phases/             # cto|ceo|cmo|cso|absorb.md 등
├── skills/vais-seo/         # deprecated → CMO 리다이렉트
├── skills/vais-validate-plugin/ # deprecated → CSO 리다이렉트
├── templates/               # Plan/Design/Architect/QA/Report 템플릿
├── package.json             # claude-plugin 메타데이터
├── vais.config.json         # 중앙 설정 (v2.0.0)
└── CHANGELOG.md
```
...
`/vais auto {기능}` → `/vais cto {기능}`으로 자동 라우팅됩니다. 기존 명령어 그대로 써도 됩니다.
</details>

<details>
<summary><strong>vais-seo와 vais-validate-plugin은 사라지나요?</strong></summary>

사라지지 않습니다. `/vais-seo`는 CMO로, `/vais-validate-plugin`은 CSO로 자동 라우팅됩니다. 기존 명령어 모두 호환됩니다.
</details>
...
네. `agents/{role}.md`를 만들고 `vais.config.json`의 `cSuite.roles`에 등록하면 됩니다. 표준 frontmatter(model, tools, hooks.Stop, disallowedTools)만 맞추면 됩니다.
</details>
