...
tech-lead가 모든 단계를 자동 진행합니다. `plan`, `fe` 단계에서 게이트 체크포인트가 작동하여 사용자 확인을 받습니다.
...
```bash
/vais status    # 현재 진행률 확인
/vais next      # 다음 할 일 안내
```

### 피처명을 생략하면?

피처명 없이 커맨드를 실행하면 기존 피처 목록에서 선택하거나, 새 피처명을 입력할 수 있습니다:
...
| 구간 | 에이전트 A | 에이전트 B |
|------|-----------|-----------|
| 설계 | designer → UI 설계 | backend-dev → DB 설계 |
| 구현 | frontend-dev → 프론트엔드 | backend-dev → 백엔드 |

### 산출물 경로

| 단계 | 산출물 경로 |
|------|-----------|
| 1. 조사·탐색 | `docs/01-research/{feature}.md` |
| 2. 기획 (+ 코딩 규칙) | `docs/02-plan/{feature}.md` |
| 3. IA | `docs/03-ia/{feature}.md` |
| 4. 와이어프레임 | `docs/04-wireframe/{feature}.md` |
| 5. UI 설계 | `docs/05-design/{feature}.md` |
| 5. DB 설계 | `docs/05-design/{feature}-db.md` |
| 8. Gap 분석 | `docs/06-check/{feature}.md` |
| 9. 검토 | `docs/07-review/{feature}.md` |
...
### 1. 자동 워크플로우 (`/vais auto`)

tech-lead 에이전트가 전체 워크플로우를 자동으로 오케스트레이션합니다:
...
- DB 불필요 시 → UI 설계만 실행
- 결과물: `{feature}.md` (UI) + `{feature}-db.md` (DB)

#### DB 종류 선택

DB 설계 시 사용할 DB 종류를 선택합니다:

| 모드 | 동작 |
|------|------|
| auto | SQLite (zero-config 기본값) |
| 수동 | AskUserQuestion으로 3가지 선택: SQLite / PostgreSQL / 외부 서비스 |

외부 서비스(Supabase, Firebase, MongoDB 등) 선택 시 `.env` 설정 안내와 함께 해당 클라이언트 코드를 생성합니다.
...
### 3. 빌드 검증 + Gap 분석 + QA (`/vais check`)

check 단계가 4단계로 강화되었습니다:

1. **빌드/실행 검증**: 의존성 → 빌드 → 서버 시작 → 핵심 동작 확인
2. **Gap 분석**: 설계 vs 구현 비교, 일치율 산출, 패치 단위 수정 지시
3. **보안 스캔**: OWASP Top 10 체크
4. **QA 시나리오 리스트 생성**: 기획서 기반 테스트 시나리오 (review에서 판정)
...
| 모드 | 동작 |
|------|------|
| auto | 프로젝트에 CSS 파일이 없으면 design 토큰 기반으로 자동 생성 |
| 수동 | CSS 파일이 없으면 AskUserQuestion으로 선택: 자동 생성 / 파일 지정 / Tailwind |
...
에이전트는 도구일 뿐이고, **제어권이 어디 있느냐**가 실행 방식의 본질입니다.
...
```json
{
  "version": "0.9.1",
  "workflow": {
    "phases": ["research", "plan", "ia", "wireframe",
               "design", "fe", "be", "check", "review"]
  },
  "parallelGroups": {
    "design": ["ui-design", "db-design"],
    "implementation": ["fe", "be"]
  },
  "chaining": {
    "sequential": ":",
    "parallel": "+"
...
| `parallelGroups` | — | 병렬 실행 가능한 단계 그룹 정의 |
...
| version | date | change |
|---------|------|--------|
| v0.7.0 | 2025-06-14 | Skills 2.0 구조 리팩토링, 에이전트 팀 시스템 |
| v0.8.0 | 2025-06-14 | 10→9단계 간소화, convention→plan 통합, docs 10→7개 |
| v0.8.1 | 2026-03-14 | QA 시나리오, /vais init, 기능·정책·화면정의서 통합 |
