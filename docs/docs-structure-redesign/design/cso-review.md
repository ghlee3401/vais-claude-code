...
> 참조: `docs/docs-structure-redesign/plan/main.md`, `docs/docs-structure-redesign/design/main.md`
...
**영향**: 단순 문자열 주석이 아니라 **phase 추출 런타임 로직**. 새 구조 `docs/{feature}/plan/main.md`는 `.plan.md` fallback으로 매치 실패 → phase 식별 불가. 방치 시 template-validator가 새 구조 문서를 검증 대상에서 누락.
...
**리스크**: 낮음 — `docs/{feature}/plan/main.md`는 `/plan/` segment로 감지 가능. 다만 fallback 분기가 **dead code**화됨.
