...
based_on: []
budget_exception:
  reason: REQ-001~014 가 각각 표를 요구하는 빠짐없는 설계 정본이며 한글 3바이트 특성으로 28KB. 사용자가 2026-09-15 '예산 예외 허용' 으로 허용
  approved_by: user
---

# Design — 하네스 설계 문서 (harness-design)
...
## 1. 요구사항별 동작 · 입력 · 출력 · 오류 · TC

| REQ | 동작 (이 문서에서 무엇을 정의하는가) | 입력 | 출력 | 오류 처리 | 절 | TC |
|---|---|---|---|---|---|---|
| REQ-001 | 문서 맨 위 한 장 요약 | 설계 전체 | 0절 | 300자 초과 시 다시 줄임 | 0 | TC-001 |
| REQ-002 | 사슬 11단계 각각의 문서 정의 | 사용자 요구 사슬 | 2.2 표 | 열 하나라도 비면 Gate 실패 | 2.2 | TC-002 |
| REQ-003 | ID 사슬 규칙 | 단계 ID 체계 | 2.3 규칙 | 부모 없는 항목 저장 거부 | 2.3 | TC-003 |
| REQ-004 | 단계별 억지력 | 손잡이 6 | 2.4 표 | 빈칸이면 Gate 실패 | 2.4 | TC-004 |
| REQ-005 | 루프·일 양식 | 기능·UI·버그 | 2.5 표 | 양식에 읽는 문서 없으면 실패 | 2.5 | TC-005 |
| REQ-006 | 제품 노트·브리핑·상태 줄 | 4면 정의 | 2.6 표 | 출처 파일 없는 면은 실패 | 2.6 | TC-006 |
| REQ-007 | 명령 다섯 | 명령 목록 | 2.7 표 | 입출력 빈 명령은 실패 | 2.7 | TC-007 |
| REQ-008 | 바탕 셋 규칙 | 기억·안전·확장 | 2.8 | 자동 기록 지점 없으면 실패 | 2.8 | TC-008 |
| REQ-009 | 대응표 | 설계 요소 전부 | 2.10 표 | 빈칸 1개라도 있으면 실패 | 2.10 | TC-009 |
| REQ-010 | 사용 장면 6개 | 장면 목록 | 2.11 | 막히는 장면이 있으면 실패 | 2.11 | TC-010 |
| REQ-011 | 커널 결함 목록 | Plan 1절 결함 | 2.12 표 | 작업 번호 없는 결함은 실패 | 2.12 | TC-011 |
| REQ-012 | 후속 로드맵 | 작업 후보 | 2.13 표 | 규모·의존 없는 작업은 실패 | 2.13 | TC-012 |
| REQ-013 | 역할 카드와 단계 결합 | 역할 23개 | 2.9 표 | owner 없는 단계는 실패 | 2.9 | TC-013 |
| REQ-014 | 별도 Agent 기준 | 세 조건 | 2.9 규칙 | 조건 밖 위임은 guard 거부 | 2.9 | TC-014 |
...
| `stage-*` (1~10) | 바로 앞 단계 문서 | 자기 ID | 2.2 표의 고르기 열 | 2.2 표의 확인 열 | 자기 정본 파일 | 최대 5회 |
...
**확장** — (1) Claude Code 와 닿는 면(hook 입력 파싱, 도구 이름, Agent 결과 모양)은 `lib/io.js` 한 곳. (2) 모든 transaction receipt 에 `runtime: {model, claudeCode, plugin}` 버전 도장. (3) `tests/regression/` 시나리오 6개(2.11 장면)를 `npm run regression` 으로 돌리고 doctor 가 버전 변화 감지 시 권고. (4) 역할 카드에 `modelHint` (judgment=strong, implementation=default, check=cheap) 데이터 필드. (5) 검사 어댑터 등록식 유지, `screenshot-compare` 어댑터 자리 예약.
...
| # | 작업 (feature slug) | 규모 | 의존 | 완료 조건 |
|---|---|---|---|---|
| H1 | `harness-health` — 결함 1~5·7, 비상 스위치, fail-loud, 읽기 허용, 버전 도장 | standard | 없음 | doctor PASS, `Enforce` 입력 시 정상, 회귀 세트 골격 |
| H2 | `chain-stages` — chain-stages.json, work-kinds.json, id-chain, `stage present`, kind 진입 조건, 산출물 직접 기록 | extended | H1 | 장면 A 가 1~10 단계 문서로 끝까지 진행 |
| H3 | `product-note` — 장부, 제품 노트 4면, 브리핑, 상태 줄, Stop 잠금, doctor | standard | H2 | 장면 E 통과, decisions.md 자동 생성 |
| H4 | `ui-loop` — ui kind, 화면 확인 정지점, screen-capture, 검수 페이지, 전/후, 취향 장부, 5회 루프 | extended | H2, H3 | 장면 C 통과 |
| H5 | `commands` — 상태·설명·저장·되돌리기·제안·기록 | standard | H3 | 명령 다섯 표대로 동작 |
| H6 | `feature-bug-kinds` — feature/bug kind, 인용 강제, stale Gate, 앱 실행 어댑터 | standard | H2, H4 | 장면 B·D 통과 |
| H7 | `regression-and-docs` — 회귀 세트 6, README/ONBOARDING/CLAUDE 갱신, 버전 4.0.0 | compact | H1~H6 | `npm run regression` PASS, 플러그인 업데이트 |
| H8 | `design-system-mcp` — mcp/design-system/vendor 를 DS 단계에 쓸지 결정·정리 | compact | H4 | 유지 또는 삭제 결정 기록 |

## 3. 전문 영역 판단

| 영역 | 판단 | 이유 |
|---|---|---|
| UI/UX | 불필요 (이 작업) | 문서 작업. UI 설계는 H4 에서 다룬다 |
| 데이터·API | 불필요 | 코드 변경 없음 |
| 보안 | 불필요 | 비밀 정보 없음, secret-scan 만 review 에서 실행 |
| 운영·배포 | 불확실 → 제외 | 플러그인 배포는 H7 에서 별도 판단 |
| 문서 품질 | 필요 | independent-qa 가 REQ-001~014 를 TC-001~014 로 검사 |

## 4. Do 위임과 쓰기 범위

| 담당 | 일 | write scope | 순서 |
|---|---|---|---|
| 본 목소리 (cto) | 승인된 이 Design 을 `docs/harness/design.md` 로 확정하고, `docs/harness/README.md` (한 장 요약 + 목차), `docs/harness/roadmap.md` (2.13) 를 만든다 | `docs/harness/**` | 1 |
| specialist | 없음 — 2.9 의 세 조건에 해당하지 않는 문서 작업이므로 별도 Agent 를 띄우지 않는다 | — | — |

## 5. Readiness · Review 검사와 QA 기대

- readiness check: `plugin-validator`.
- review check: `plugin-validator`, `secret-scan`.
- independent-qa 검사 목록:

| TC | 검사 | 기대 |
|---|---|---|
| TC-001 | 0절 글자 수 | 공백 포함 300자 이내이며 사슬·억지력·역할이 언급됨 |
| TC-002 | 2.2 표 | 11행 모두 8열이 채워짐 |
| TC-003 | 2.3 | 부모 허용표·stale·인용 강제 세 규칙 존재 |
| TC-004 | 2.4 두 표 | 공통 7행·단계별 11행 빈칸 0, 손잡이 6개 전부 배정 |
| TC-005 | 2.5 표 | kind 4행에 읽는 문서·ID·갱신 문서 존재 |
| TC-006 | 2.6 표 | 4면 모두 출처 파일과 갱신 시점 존재 |
| TC-007 | 2.7 표 | 5행에 입력·출력·읽기/쓰기 존재 |
| TC-008 | 2.8 | 자동 기록 지점 ≥5, 비상 스위치, 버전 도장, 회귀 세트 언급 |
| TC-009 | 2.10 표 | 모든 행에 파일과 상태 존재, 2.2~2.9 의 요소가 빠짐없이 등장 |
| TC-010 | 2.11 | 장면 6개가 각각 Report 또는 경고로 종결 |
| TC-011 | 2.12 표 | Plan 1절 결함 전부에 H 번호 |
| TC-012 | 2.13 표 | 8행에 규모·의존·완료 조건 |
| TC-013 | 2.9 표 | 단계 11 + Review 에 owner 존재 |
| TC-014 | 2.9 규칙 | 세 조건과 guard 거부 문장 존재 |

## 6. 롤백

이 작업은 `docs/harness/**` 만 만든다. 롤백은 해당 폴더 삭제와 Work item 취소로 충분하며 코드·설정·상태 머신은 변하지 않는다. 설계 내용의 롤백은 새 Work item 으로 Design revision 을 올린다.
