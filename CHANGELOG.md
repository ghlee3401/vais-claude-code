...
- **`lib/mcp-validator.js`** (162 LOC, 5 export) — `isMcpEnabled` / `validateEnvironment` / `runGenerate` / `buildErrorMessage` / `hasMasterDoc` API. Profile gate 와 동일 fallback 패턴 (config 키 누락 시 true)
- **`hooks/design-mcp-trigger.js`** (107 LOC) — PreToolUse Agent matcher 훅. ui-designer 진입 + activeFeature.currentPhase === "design" 매칭 시 검증 + MASTER.md idempotency + generate 자동 호출
- **`mcp/design-system-server-runner.js`** (188 LOC, observation-D1) — Claude Code MCP server (stdio JSON-RPC 2.0) wrapping `mcp/design-system-server.json` 의 3 tool (`design_search` / `design_system_generate` / `design_stack_search`). SDK 미사용 raw JSON-RPC 구현
- **`.mcp.json`** (project root) + `.claude-plugin/plugin.json > mcpServers` 양쪽 등록 (dev + 배포 자동 인식)
- **신규 17 통합 테스트** — `tests/integration/mcp-validator.test.js` (TC-1~TC-6 + 3 추가) + `tests/integration/mcp-migration.test.js` (TC-A~TC-H). Module._load mock 으로 fs/child_process/paths 격리
- **신규 의존성 없음** — Node.js 표준 lib 만 사용 (readline, child_process). Python3 ≥ 3.10 은 vendor/ui-ux-pro-max 의 runtime 의존 (MCP 활성 시 Hard fail 검증)
- 새 피처 산출물 `docs/design-system-mcp-activation/` (00-ideation + 01-plan + 02-design + 03-do, 4 main + 3 topic = 7 docs)

### Changed

- **`vais.config.json > orchestration.mcp.enabled`** = `true` (기본 ON, v0.61.0+). 키 누락 시 fallback 도 true (`lib/mcp-validator.js`)
- **`agents/cto/ui-designer.md`** — `tools` 에 `mcp__vais-design-system__design_search` + `design_system_generate` 추가. description 정정: "Consumes design tokens generated by UI/UX Pro Max" → **"Auto-generates design tokens via MCP and consumes"** (D-13, 호출 주체 명시)
- **`agents/cto/frontend-engineer.md`** — `tools` 에 `mcp__vais-design-system__design_stack_search` 추가 (do phase 구현 직전 가이드)
- **`hooks/hooks.json`** — PreToolUse 에 Agent matcher → `design-mcp-trigger.js` 등록 (timeout 30s — vendor Python subprocess 여유)
- **`README.md`** — Quick Start#Requirements 신설, Python3 ≥ 3.10 의존 + Hard fail 안내 + opt-out 가이드
- **`CLAUDE.md`** — 의존성 절 신설 (Node.js / Python3) + `vais.config.json > orchestration.mcp.enabled` 설명

### Why (전환 사유)

- v0.48 부터 mcp/design-system-server.json 스펙 + vendor/ui-ux-pro-max 데이터 + ui-designer.md "Consumes design tokens" description 모두 있었으나 MCP 자체가 비활성 (mcp.enabled=false, .mcp.json 없음, agent tool 권한 없음, plugin.json 등록 없음) → ui-designer 가 token 없이 디자인하거나 사용자 수동 호출 필요
- 12 minor 미뤄온 사유 = 우선순위 (다른 곳이 바빠서). 기술적/품질적 blocker 없음
- v0.60.0 Profile gate ON 결정과 같은 패턴 — sub-agent / mcp 서버 / 템플릿이 모두 갖춰진 시점에서 default 활용이 정석
- Hard fail 채택: graceful skip 은 사일런트 품질 저하 (token 없이 design 진행) 유발. 명시적 안내 + opt-out 경로 보장이 더 친절

### Migration

- 신규 install: 자동 ON (Python3 ≥ 3.10 필수)
- 기존 v0.60.0 사용자 (mcp 키 없음): fallback 도 true 로 자동 ON
- 기존 사용자 (`mcp.enabled: false` 명시): opt-out 존중, 수동 true 변경 시 활성
- opt-out: `vais.config.json > orchestration.mcp.enabled: false`

### Decision Record

- D-1 ~ D-4 (ideation, ceo): scope/value/trigger/fallback 4 결정
- D-5 ~ D-13 (plan, cto): tool 권한 분배 / 호출 시점 / idempotency / Python 버전 / vendor 검증 / 마이그 / 안내 / 테스트 / description 정정
- D-D1 ~ D-D7 (design, cto): Architecture C.Pragmatic / .mcp.json 위치 / hook 등록 / API 시그니처 / 매칭 조건 / MASTER.md 경로 / 에러 전파
- D-IM-01 ~ D-IM-04 (do, cto): hook timeout 30s / readline JSON-RPC / 입력 검증 중복 (defense in depth) / .mcp.json + plugin.json 양쪽 등록
- 총 **24 결정** 누적 (4 + 9 + 7 + 4)

### Testing

- `npm test` — 263 → **280 pass** (+17 신규), fail 0, skip 3
- `node --test tests/integration/mcp-{validator,migration}.test.js` — 17/17 pass
- `node scripts/vais-validate-plugin.js` — vais-code@0.61.0 정상, MCP 서버 인식

### Follow-up (qa phase 검증 필요)

- F-1: Python 3.10 minimum 실 검증 (`vendor/ui-ux-pro-max/scripts/search.py` 의존 측정)
- F-2: manual smoke (실 design phase 호출 → MASTER.md 생성 확인)
- F-3: `.mcp.json` + `plugin.json > mcpServers` 양쪽 등록 충돌 여부
- F-4: activeFeature 가 status.json 에 없을 때 동작 확인
