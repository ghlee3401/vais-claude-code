...
| ~~`lib/advisor/wrapper.js`~~ | sub-plan 06에서 진입점 추가 | `tests/advisor-integration.test.js` | **유지** (CP-3 (B)) |
| ~~`lib/advisor/prompt-builder.js`~~ | wrapper에서 호출 | `tests/advisor-integration.test.js` | **유지** |
| ~~`lib/advisor/index.js`~~ | export 진입점 | 없음 | **유지** |
...
- **Advisor 유지 (CP-3 B, 사용자 결정 2026-04-17)**: 본 sub-plan에서는 advisor 관련 모듈은 **건드리지 않는다**. sub-plan 06에서 진입점(CLI + session-start 모드 판정 + cost-monitor 연결)을 추가하여 활성화한다.
...
**유지** (sub-plan 06에서 처리): `lib/advisor/`, `lib/control/cost-monitor.js`
...
**유지**: `tests/advisor-integration.test.js`, `tests/advisor-degrade.test.js` (sub-plan 06에서 활용)
...
`vais.config.json > advisor` 섹션은 **유지** (sub-plan 06에서 wrapper/cost-monitor가 소비).
agent markdown frontmatter의 `advisor.enabled: true` 필드도 **유지** (native 모드에서 Claude Code가 자체 해석).
...
| # | 검증 | 방법 |
|---|------|------|
| V-1 | `lib/quality/`, `lib/registry/`, `lib/validation/` 부재. `lib/advisor/` 존재 (06에서 활성화) | `ls` |
...
| V-7 | `lib/advisor/` + `lib/control/cost-monitor.js` + 관련 advisor 테스트 그대로 존재 (06에서 활성화) | `ls` |
...
- **sub-plan 04**: `vais.config.json`의 `advisor` 섹션은 **유지** (06이 소비). 다른 dead 키만 제거.
- **sub-plan 05**:
  - CHANGELOG에 state-machine 교체 내역 기록
  - tests/ 재구성 목록 명시 (삭제된 5개 테스트 반영)
  - CLAUDE.md의 Advisor Tool(M-24) 섹션은 06 결과 반영하여 갱신
- **sub-plan 06**: `scripts/check-cc-advisor-support.js`(현존), `scripts/patch-advisor-frontmatter.js`(현존) 사용/유지 결정
